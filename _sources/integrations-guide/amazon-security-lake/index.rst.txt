...
.. note::
   ...
   This document guides you through setting up Cyb3rhq as a data source for AWS Security Lake. To configure Cyb3rhq as a subscriber to Amazon Security Lake, refer to :doc:`Cyb3rhq as a subscriber </cloud-security/amazon/services/supported-services/security-lake>`.

Cyb3rhq Security Events can be converted to OCSF events and Parquet format, required by Amazon Security Lake, by using an AWS Lambda Python function, a Logstash instance and an AWS S3 bucket.

A properly configured Logstash instance can send the Cyb3rhq Security events to an AWS S3 bucket, automatically invoking the AWS Lambda function that will transform and send the events to the Amazon Security lake dedicated S3 bucket.

The diagram below illustrates the process of converting Cyb3rhq Security Events to OCSF events and to Parquet format for Amazon Security Lake.
...
-  At least one up and running Cyb3rhq Indexer instance with populated ``cyb3rhq-alerts-4.x-*`` indices.
-  A Logstash instance.
...
Follow the `official documentation <https://docs.aws.amazon.com/AmazonS3/latest/userguide/create-bucket-overview.html>`__ to create an S3 bucket within your organization. Use a descriptive name, for example: ``cyb3rhq-aws-security-lake-raw``.

Creating a Custom Source in Amazon Security Lake
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

Configure a custom source for Amazon Security Lake via the AWS console. Follow the `official documentation <https://docs.aws.amazon.com/security-lake/latest/userguide/custom-sources.html>`__ to register Cyb3rhq as a custom source.
...
#. Enter a descriptive name for your custom source. For example, ``cyb3rhq``.
...
.. code-block:: console
   ...
   $ touch "$(date +'%Y%m%d')_ls.s3.cyb3rhq-test-events.$(date +'%Y-%m-%dT%H.%M').part00.txt"

Add the sample events below to the file and upload it to the S3 bucket.

.. code-block:: JSON

   {"cluster":{"name":"cyb3rhq-cluster","node":"cyb3rhq-manager"},"timestamp":"2024-04-22T14:20:46.976+0000","rule":{"mail":false,"gdpr":["IV_30.1.g"],"groups":["audit","audit_command"],"level":3,"firedtimes":1,"id":"80791","description":"Audit: Command: /usr/sbin/crond"},"location":"","agent":{"id":"004","ip":"47.204.15.21","name":"Ubuntu"},"data":{"audit":{"type":"NORMAL","file":{"name":"/etc/sample/file"},"success":"yes","command":"cron","exe":"/usr/sbin/crond","cwd":"/home/cyb3rhq"}},"predecoder":{},"manager":{"name":"cyb3rhq-manager"},"id":"1580123327.49031","decoder":{},"@version":"1","@timestamp":"2024-04-22T14:20:46.976Z"}
   {"cluster":{"name":"cyb3rhq-cluster","node":"cyb3rhq-manager"},"timestamp":"2024-04-22T14:22:03.034+0000","rule":{"mail":false,"gdpr":["IV_30.1.g"],"groups":["audit","audit_command"],"level":3,"firedtimes":1,"id":"80790","description":"Audit: Command: /usr/sbin/bash"},"location":"","agent":{"id":"007","ip":"24.273.97.14","name":"Debian"},"data":{"audit":{"type":"PATH","file":{"name":"/bin/bash"},"success":"yes","command":"bash","exe":"/usr/sbin/bash","cwd":"/home/cyb3rhq"}},"predecoder":{},"manager":{"name":"cyb3rhq-manager"},"id":"1580123327.49031","decoder":{},"@version":"1","@timestamp":"2024-04-22T14:22:03.034Z"}
   {"cluster":{"name":"cyb3rhq-cluster","node":"cyb3rhq-manager"},"timestamp":"2024-04-22T14:22:08.087+0000","rule":{"id":"1740","mail":false,"description":"Sample alert 1","groups":["ciscat"],"level":9},"location":"","agent":{"id":"006","ip":"207.45.34.78","name":"Windows"},"data":{"cis":{"rule_title":"CIS-CAT 5","timestamp":"2024-04-22T14:22:08.087+0000","benchmark":"CIS Ubuntu Linux 16.04 LTS Benchmark","result":"notchecked","pass":52,"fail":0,"group":"Access, Authentication and Authorization","unknown":61,"score":79,"notchecked":1,"@timestamp":"2024-04-22T14:22:08.087+0000"}},"predecoder":{},"manager":{"name":"cyb3rhq-manager"},"id":"1580123327.49031","decoder":{},"@version":"1","@timestamp":"2024-04-22T14:22:08.087Z"}
...
Install Logstash on a dedicated server or on the server hosting the Cyb3rhq Indexer. Logstash forwards the data from the Cyb3rhq Indexer to the AWS S3 bucket created previously.
...
#. Copy the Cyb3rhq Indexer root certificate on the Logstash server, to any folder of your choice (e.g, ``/usr/share/logstash/root-ca.pem``).
#. Give the ``logstash`` user the required permissions to read the certificate.

   .. code-block:: console

      $ sudo chmod -R 755 </PATH/TO/CYB3RHQ_INDEXER/CERTIFICATE>/root-ca.pem

Configuring the Logstash pipeline
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

A `Logstash pipeline <https://www.elastic.co/guide/en/logstash/current/configuration.html>`__ allows Logstash to use plugins to read the data from the Cyb3rhq Indexer and send them to an AWS S3 bucket.

The Logstash pipeline requires access to the following secrets:

-  Cyb3rhq Indexer credentials: ``INDEXER_USERNAME`` and ``INDEXER_PASSWORD``.
   ...
   .. code-block:: ruby
      ...
      input {
          opensearch {
              ...
              ssl => true
              ca_file => "</PATH/TO/CYB3RHQ_INDEXER/CERTIFICATE>/root-ca.pem"
...
The integration maps Cyb3rhq Security Events to the *OCSF v1.1.0* `Security Finding (2001) <https://schema.ocsf.io/classes/security_finding>`__ Class.

The tables below represent how the Cyb3rhq Security Events are mapped into the OCSF Security Finding Class.
...
+------------------------------+---------------------+--------------------+
| metadata.product.name        | String              | "Cyb3rhq"            |
+------------------------------+---------------------+--------------------+
| metadata.product.vendor_name | String              | "Cyb3rhq, Inc."      |
+------------------------------+---------------------+--------------------+
...
+------------------------------+---------------------+--------------------+
| metadata.log_provider        | String              | "Cyb3rhq"            |
+------------------------------+---------------------+--------------------+
...
+------------------------+---------------------+----------------------------------------+
| **OCSF Key**           | **OCSF Value Type** | **Cyb3rhq Event Value**                  |
+------------------------+---------------------+----------------------------------------+
