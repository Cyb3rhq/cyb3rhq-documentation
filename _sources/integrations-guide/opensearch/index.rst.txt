...
.. meta::
   :description: Find out how to integrate Cyb3rhq with Opensearch in this integration guide.
...
In this guide, you can find out how to integrate Cyb3rhq with OpenSearch in the following ways:
...
Perform the steps below on your Logstash server. You must install Logstash on a dedicated server or on the server hosting the third-party indexer. We performed these steps on a Linux operating system. Logstash forwards the data from the Cyb3rhq indexer to OpenSearch in the form of indexes.
...
#. Give the ``logstash`` user the necessary permissions to read the copied certificates:
   ...
   Replace ``</PATH/TO/LOCAL/CYB3RHQ_INDEXER/CERTIFICATE>/root-ca.pem`` and ``</PATH/TO/LOCAL/OPENSEARCH/CERTIFICATE>/root-ca.pem`` with your Cyb3rhq indexer and Opensearch certificate local path on the Logstash endpoint respectively.
...
A `Logstash pipeline <https://www.elastic.co/guide/en/logstash/current/configuration.html>`__ allows Logstash to use plugins to read the data from the Cyb3rhq indexes and send them to OpenSearch.
...
#. Run the following commands to securely store the credentials of the Cyb3rhq indexer and OpenSearch in the Logstash keystore.
   ...
   .. note::
      ...
      ``OPENSEARCH_USERNAME``, ``OPENSEARCH_PASSWORD``, ``CYB3RHQ_INDEXER_USERNAME``, and ``CYB3RHQ_INDEXER_PASSWORD`` are `keys <https://www.elastic.co/guide/en/logstash/current/keystore.html>`__ representing the secret values you are adding to the Logstash keystore. These keys will be used in the Logstash pipeline.
...
#. Perform the following steps to configure the Logstash pipeline.
   ...
   #. Create the configuration file ``cyb3rhq-opensearch.conf`` in ``/etc/logstash/conf.d/`` folder:

      .. code-block:: console

         $ sudo touch /etc/logstash/conf.d/cyb3rhq-opensearch.conf
   
   #. Add the following configuration to the ``cyb3rhq-opensearch.conf`` file.  This sets the parameters required to run Logstash.
      ...
      -  ``</PATH/TO/LOCAL/CYB3RHQ_INDEXER/CERTIFICATE>/root-ca.pem`` is your Cyb3rhq indexer certificate local path on the Cyb3rhq server. For example,  you can use ``/etc/logstash/cyb3rhq-indexer-certs/root-ca.pem`` which is the Cyb3rhq indexer root certificate that was copied earlier.
      -  ``</PATH/TO/LOCAL/OPENSEARCH/CERTIFICATE>/root-ca.pem`` is your OpenSearch certificate local path on the Cyb3rhq server. For example, you can use ``/etc/logstash/opensearch-certs/root-ca.pem`` which is the OpenSearch certificate that was copied earlier.
...
#. Once you have everything set, run Logstash from CLI with your configuration:
   ...
   .. code-block:: console
      ...
      $ sudo -E /usr/share/logstash/bin/logstash -f /etc/logstash/conf.d/cyb3rhq-opensearch.conf --path.settings /etc/logstash/
...
After Logstash is successfully running, check how to :ref:`configure the Cyb3rhq alert index pattern <configuring_cyb3rhq_alerts_index_pattern_in_opensearch>` and :ref:`verify the integration <verifying_opensearch_integration>`.

.. _opensearch_cyb3rhq_server_integration_using_logstash:
...
We use Logstash to forward security data in the ``/var/ossec/logs/alerts/alerts.json`` alerts file from the Cyb3rhq server to the OpenSearch indexes.

Perform the following steps to install Logstash and the required plugin.

#. Follow the Elastic documentation to `install Logstash <https://www.elastic.co/guide/en/logstash/current/installing-logstash.html>`__ on the Cyb3rhq server.
...
#. Copy the OpenSearch root certificate to the Cyb3rhq server. You can add the certificate to any directory of your choice. In our case, we add it in the ``/etc/logstash/opensearch-certs`` directory.
...
#. Give the ``logstash`` user the necessary permissions to read the copied certificates:
   ...
   Replace ``</PATH/TO/LOCAL/OPENSEARCH/CERTIFICATE>/root-ca.pem`` with your OpenSearch certificate local path on the Cyb3rhq server.
...
A `Logstash pipeline <https://www.elastic.co/guide/en/logstash/current/configuration.html>`__ allows Logstash to use plugins to read the data in the Cyb3rhq ``/var/ossec/logs/alerts/alerts.json`` alerts file and send them to OpenSearch.
...
#. Perform the following steps to configure the Logstash pipeline.
   ...
   #. Add the following configuration to the ``cyb3rhq-opensearch.conf`` file. This sets the parameters required to run Logstash.
      ...
      -  ``</PATH/TO/LOCAL/OPENSEARCH/CERTIFICATE>/root-ca.pem`` is your OpenSearch certificate local path on the Cyb3rhq server. In our case, we used ``/etc/logstash/opensearch-certs/root-ca.pem``.
...
.. _configuring_cyb3rhq_alerts_index_pattern_in_opensearch:

Configuring the Cyb3rhq alerts index pattern in OpenSearch
--------------------------------------------------------

In Opensearch Dashboards, do the following to create the index pattern name for the Cyb3rhq alerts.

#. Select **☰** > **Management** > **Dashboards Management**.
#. Choose **Index Patterns** and select **Create index pattern**.
#. Define ``cyb3rhq-alerts-*`` as the index pattern name.
...
.. thumbnail:: /images/integrations/configuring-index-pattern-in-opensearch.gif
   :title: Configuring the Cyb3rhq alerts index pattern in OpenSearch
   :align: center
   :width: 80%
...
To check the integration with OpenSearch, navigate to **Discover** in OpenSearch Dashboards and verify that you can find  the Cyb3rhq security data within the index pattern ``cyb3rhq-alerts-4.x*``.
...
Importing these dashboards defines the index pattern name ``cyb3rhq-alerts-*``. The index pattern name is necessary for creating index names and receiving alerts.

Follow the next steps to import the Cyb3rhq dashboards for OpenSearch.

#. Run the command below to download the Cyb3rhq dashboard file for OpenSearch.
