...
.. meta::
   :description: Alerts are notifications generated by the Cyb3rhq manager after processing events received from Cyb3rhq agents and agentless devices. Learn more in this section of the documentation.

Alert management
================

Alerts are notifications generated by the Cyb3rhq manager after processing events received from Cyb3rhq agents and agentless devices. By default, the alerts are stored in the ``/var/ossec/logs/alerts/alerts.log`` and ``/var/ossec/logs/alerts/alerts.json`` files.

By default, the Cyb3rhq server uses Filebeat to forward generated alerts to the Cyb3rhq indexer for indexing. Additionally, you can configure the Cyb3rhq manager to forward alerts to other systems involving syslog servers, email systems, and databases.
...
An alert threshold is the minimum severity level that must be exceeded for an alert to be triggered. The Cyb3rhq manager assigns a severity level to each event from monitored endpoints based on the matching rule from the ruleset. By default, it only triggers alerts with a severity level of ``3`` or higher.

Configuration
^^^^^^^^^^^^^

The alert threshold is configured in the ``/var/ossec/etc/ossec.conf`` configuration file on the Cyb3rhq server within the ``<alerts>`` XML tag.
...
.. note::
   ...
   Restart the Cyb3rhq manager when you make any changes to the configuration file. This action ensures that the changes take effect.
...
The Cyb3rhq manager forwards alerts to the Cyb3rhq indexer by default for indexing and analytics capabilities. Additionally, the Cyb3rhq manager provides the capability of configuring and forwarding alerts to other systems for analysis and backup.
...
You can configure the Cyb3rhq server to send alerts to a syslog server using the :ref:`syslog_output <reference_ossec_syslog_output>` option. Forwarding alerts to a syslog server can be useful for centralized monitoring and custom reporting.

Configuration
~~~~~~~~~~~~~

Syslog output is configured in the Cyb3rhq server ``/var/ossec/etc/ossec.conf`` configuration file within the ``<ossec_config>`` block. By default, the Cyb3rhq manager forwards alerts to syslog servers using port ``514`` via the UDP protocol.
...
-  The ``<level>`` tag sets the minimum severity level of the alerts to be forwarded to the syslog server. The example value ``9`` indicates the Cyb3rhq server only forwards alerts to the syslog server if the alert level is higher than ``9``. If this option is not defined, the Cyb3rhq server forwards all alerts to the syslog server.
...
Restart the Cyb3rhq manager service to apply the changes after every configuration:
...
Cyb3rhq provides a feature to send alerts to email systems when they are generated on a Cyb3rhq server. You can configure it to send email alerts to one or more email addresses when rules are triggered or based on customized settings. This configuration can help you for daily event reports and more.

A sample email sent by Cyb3rhq when the rule ID ``553`` is triggered is shown below:
...
.. code-block:: none
   ...
   Received From: cyb3rhq-server->syscheck
...
To configure Cyb3rhq to send email alerts, we configure the email options within the ``<global>`` section of the ``/var/ossec/etc/ossec.conf`` file.
...
.. warning::
   ...
   Cyb3rhq doesn't handle SMTP authentication. If your email service uses this, you need to :ref:`configure a server relay <smtp_server_with_authentication>`.
...
Cyb3rhq allows granular configuration options for email alerts. This setting extends the `generic email options`_ configured in the ``<global>`` section of the ``/var/ossec/etc/ossec.conf`` file. Granular email configurations are defined within the ``<email_alerts>`` tag of the ``/var/ossec/etc/ossec.conf`` file.
...
.. warning::
   ...
   The minimum severity level configured in the ``<alerts>`` section applies to and overrides these granular email configurations. For example, if you configure the Cyb3rhq manager to send an email when rule ``526`` is triggered, but the rule has a level lower than the minimum level specified in the ``<alerts>`` section, the alert will not be sent.

Email alert based on level
''''''''''''''''''''''''''

This option configures the Cyb3rhq manager to send email alerts when the severity level is equal to or greater than the set value. This option is configured as follows:
...
This configuration allows the Cyb3rhq manager to send an email to ``you@example.com`` when any rule with a level greater than or equal to ``4`` is triggered.
...
The ``event_location`` option involves sending email alerts based on the location from where the event originated. The generated alert must match the event location to be forwarded via email. The allowed values for this option are the Cyb3rhq agent name, hostname, IP address, or log file.
...
This configuration allows the Cyb3rhq manager to send an email to ``you@example.com`` when the events that generated the alerts originated on the Cyb3rhq agent with name ``server1``.
...
This configuration allows the Cyb3rhq manager to send an email to ``you@example.com`` when the rules ``515`` or ``516`` are triggered.
...
This configuration allows the Cyb3rhq manager to send an email to ``you@example.com`` when any rule that is part of the ``pci_dss_10.6.1`` group is triggered on any Cyb3rhq monitored endpoint.
...
The minimum severity level to send an alert via email is ``12`` by default. You can configure the Cyb3rhq manager to forcefully send an email alert below the configured minimum severity level. To do so, you need to use one of the :ref:`rule <rules_options>` options below:
...
Cyb3rhq email alerts do not support SMTP servers with authentication, such as Gmail. However, you can send these emails via a server relay like Postfix.
...
#. Configure email notifications within the ``<global>`` tag of the Cyb3rhq server’s ``/var/ossec/etc/ossec.conf`` file as follows:
...
Cyb3rhq supports forwarding alerts to database systems. You can configure the Cyb3rhq manager to output generated alerts into a database. To achieve this configuration, you must compile the Cyb3rhq manager from sources with the database type that you want to use. Cyb3rhq currently supports MySQL and PostgreSQL databases.
...
You need to install the development libraries for the database system you want to configure and compile the Cyb3rhq manager to use the required database system.
...
#. Download and extract the latest version of Cyb3rhq:

   .. code-block:: console

      # curl -Ls https://github.com/cyb3rhq/cyb3rhq/archive/v|CYB3RHQ_CURRENT|.tar.gz | tar zx

#. Execute the following commands to switch to the Cyb3rhq directory and specify the database type to use, replacing the ``<DATABASE_TYPE>`` variable with either ``mysql`` or ``pgsql``:

   .. code-block:: console

      # cd cyb3rhq-|CYB3RHQ_CURRENT|/src
...
#. Run the ``install.sh`` script. It displays a wizard to guide you through the installation process using the Cyb3rhq sources:
...
#. The installer asks if you want to start Cyb3rhq at the end of the installation. If you choose not to, you can start it later with the command below:
...
.. note::
   ...
   You will be prompted twice to provide a password when creating the user. Note this password as it is required when configuring the Cyb3rhq manager.

Cyb3rhq manager configuration
~~~~~~~~~~~~~~~~~~~~~~~~~~~

Perform the following steps to configure the Cyb3rhq manager to send alerts and other data to the database system.

#. Add the following block of code within the ``<ossec_config>`` block of the ``/var/ossec/etc/ossec.conf`` file on the Cyb3rhq server:
...
#. Run the command below to verify that the Cyb3rhq manager is connected to the database:

   .. code-block:: console

      # grep cyb3rhq-dbd /var/ossec/logs/ossec.log

   .. code-block:: none
      :class: output

      2024/06/24 14:49:11 cyb3rhq-dbd: INFO: Connected to database 'Alerts_DB' at '127.0.0.1'.

The database will now start receiving data from the Cyb3rhq manager.
