...
Cyb3rhq provides two modules for monitoring the output of system commands executed on an endpoint. The Command and the Logcollector modules periodically run and monitor commands or executables on Windows, Linux, and macOS endpoints.
...
- **Encrypted communication**: All messages exchanged between the Cyb3rhq server and agents are encrypted using AES encryption. This implies that every command output from the Cyb3rhq agents is sent securely to the  Cyb3rhq server.

- **Scheduling execution**: The Command module is flexible and can be configured to run commands on endpoints as follows:

   - Immediately after the Cyb3rhq agent starts.
...
|                    |               | - Path to a script                               |                                                                                                                                                                                                                                                                                                   |                                                                                                                                         |
+--------------------+---------------+--------------------------------------------------+---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+-----------------------------------------------------------------------------------------------------------------------------------------+
|ignore_output       | no            | yes, no                                          | Specifies whether the output of a command is ignored. When this option is set to ``yes``, the command output is not forwarded to the Cyb3rhq server.                                                                                                                                                |                                                                                                                                         |
+--------------------+---------------+--------------------------------------------------+---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+-----------------------------------------------------------------------------------------------------------------------------------------+
...
| **Scheduling options**                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
+--------------------+---------------+--------------------------------------------------+---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+-----------------------------------------------------------------------------------------------------------------------------------------+
| run_on_start       | yes           | yes, no                                          | Runs the configured command immediately when the Cyb3rhq service starts.                                                                                                                                                                                                                            |                                                                                                                                         |
+--------------------+---------------+--------------------------------------------------+---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+-----------------------------------------------------------------------------------------------------------------------------------------+
...
To use the Cyb3rhq command monitoring capability, you first need to configure the system to monitor the command's output. This can be a central configuration on the Cyb3rhq server or locally on the endpoints. Once the command is configured, you can then create custom rulesets to process the output and trigger an alert.
...
#. Add the following configuration to the Cyb3rhq agent ``C:\Program Files (x86)\ossec-agent\ossec.conf`` file:
...
#. Restart the Cyb3rhq agent to apply the changes, using PowerShell with administrator privileges:
...
Configure the Cyb3rhq server with a custom decoder and rule to analyze the events received from the Windows endpoint by following the steps below.
...
Visit the Cyb3rhq dashboard and navigate to the **Threat Hunting** module to visualize the generated alert as shown below.
...
#. Add the configuration below within the ``</ossec_config>`` block of the Cyb3rhq agent’s :ref:`local configuration file <command_monitoring_local_configuration>`, ``/var/ossec/etc/ossec.conf``:
...
After you perform the steps above, the Cyb3rhq server can now analyze the output of the command, ``free -m | awk 'NR==2{printf "%.2f\t\t\n", $3*100/$2``, and trigger security alerts every ``120`` seconds.
...
As shown in the figure above, the alert is expanded to show the available fields. The ``full_log`` field contains the message received from the Linux endpoint. The value ``76.42`` in the ``full_log`` field represents the percentage of memory utilized by the Linux endpoint. This value can be captured in a variable and displayed in the message description on the Cyb3rhq dashboard. Visit the section on :ref:`Creating a custom ruleset <command_monitoring_creating_custom_ruleset>` to see how this is done.

Custom ruleset
~~~~~~~~~~~~~~

Cyb3rhq provides an out-of-the-box decoder and rule to analyze the output of commands executed with the Logcollector module to generate security alerts.

The decoder is known as ``ossec`` as shown below and is available in the ``/var/ossec/ruleset/decoders/0200-ossec_decoders.xml`` file on the Cyb3rhq server.
...
The rule is defined with ID ``530`` as shown below and is available in the ``/var/ossec/ruleset/rules/0015-ossec_rules.xml`` file on the Cyb3rhq server.
...
The default decoder and rule are limited and do not provide coverage for all available scenarios. For this reason, Cyb3rhq offers you the ability to create custom decoders and rules to meet your specific use case. Kindly visit the :ref:`Creating a custom ruleset <command_monitoring_creating_custom_ruleset>` section of this documentation for more information.

Configuration files
-------------------

Cyb3rhq agents can be configured to run and monitor the output of commands in either one of the following ways:

- Locally on each Cyb3rhq agent using the local configuration file.

- Centrally on the Cyb3rhq server using the centralized configuration file.
...
The commands to run and monitor their output can be configured in the :doc:`local configuration (ossec.conf) </user-manual/reference/ossec-conf/index>` file of individual Cyb3rhq agents on an endpoint. We use this file to manage the configuration of specified endpoints.
...
The centralized configuration file is used to remotely manage a group of endpoints. Unlike the local configuration file, it is used to manage one or more endpoints. The Cyb3rhq server runs commands on monitored endpoints remotely using the ``Remoted`` daemon. We can enable this functionality by configuring the commands to be monitored in the appropriate configuration section of the :doc:`centralized configuration (agent.conf) </user-manual/reference/centralized-configuration>` file on the Cyb3rhq server.

By default, the Cyb3rhq agents can not accept remote commands configured on the Cyb3rhq server. The remote command execution feature is disabled on the agents for security reasons. You must explicitly configure the Cyb3rhq agents to accept remote commands from the Cyb3rhq server. 

The configuration depends on the module in use, as described below:

- When using the Command module, add the configuration :ref:`cyb3rhq_command.remote_commands <cyb3rhq_command_remote_commands>` to the ``local_internal_options.conf`` file of the Cyb3rhq agent on every monitored endpoint. This configuration enables the Cyb3rhq agents to accept remote commands from the Cyb3rhq server.

- When using the Logcollector module, add the configuration :ref:`logcollector.remote_commands <ossec_internal_logcollector>` to the ``local_internal_options.conf`` file of the Cyb3rhq agent on every monitored endpoint. This configuration enables the Cyb3rhq agents to accept remote commands from the Cyb3rhq server.

.. warning:: Enable remote command execution with caution, as this action authorizes the Cyb3rhq user to run commands with elevated privileges on the monitored endpoint.

The location for the Cyb3rhq agent ``local_internal_options.conf`` file on each monitored endpoint is shown in the table below.
...
The configurations below shows how to enable and disable remote command execution on Cyb3rhq agents in Linux endpoints.

.. note:: These configurations require you to restart the Cyb3rhq agent to apply the changes.

- To enable the Cyb3rhq agent to accept remote commands from a Cyb3rhq server, add the configuration below to the ``/var/ossec/etc/local_internal_options.conf`` file on the Linux endpoint.

   - For the Command module:

      .. code-block:: xml

         cyb3rhq_command.remote_commands=1
...
- To disable remote command execution after explicitly enabling it, remove the configuration entirely or modify the appropriate value to ``0`` in the ``/var/ossec/etc/local_internal_options.conf`` file.
   ...
   - For the Command module:
      ...
      .. code-block:: xml
         ...
         cyb3rhq_command.remote_commands=0
