...
.. meta::
    :description: Configure Cyb3rhq agents to monitor processes on Windows, Linux, and macOS. Create custom rules for alerting if a given process is running or not.

Monitoring running processes
=============================

In this use case, we configure Cyb3rhq agents to run commands locally to monitor running processes on Windows, Linux, and macOS endpoints. Then, we create custom rules on the Cyb3rhq server to generate alerts when a particular process is running or not.
...
#. Append the following configuration to the Cyb3rhq agent’s ``C:\Program Files (x86)\ossec-agent\ossec.conf`` file. The ``tasklist`` command lists running processes on Windows endpoints:
...
Configure the Cyb3rhq server with a custom decoder and rule to analyze the events received from the Windows endpoint using the steps below.
...
Go to **Threat Hunting** module on the Cyb3rhq dashboard to visualize the generated alerts when the monitored command runs.      
...
Configure this endpoint to monitor the status of the running processes every two minutes and forward its output to the Cyb3rhq server for analysis.

#. Append the Logcollector module configuration below to the Cyb3rhq agent ``/var/ossec/etc/ossec.conf`` file:
...
Configure the Cyb3rhq server with custom rules to analyze the events received from the Linux endpoint using the steps below.
...
Go to **Threat Hunting** module on the Cyb3rhq dashboard to visualize the generated alert when the Cron process is not running.
...
Perform the following steps to configure the Cyb3rhq Logcollector module to check if the calendar application ``Calendar.app`` or ``calendar.app`` is running.

#. Append the following configuration to the Cyb3rhq agent ``/Library/Ossec/etc/ossec.conf`` file:
...
Configure the Cyb3rhq server with a custom rule to analyze the Calendar application events received from the macOS endpoint using the steps below.

#. Add the following rules to the ``/var/ossec/etc/rules/local_rules.xml`` file on the Cyb3rhq server. The rule only matches the string ``ossec: output: 'check_calendar_status'`` in the command output as only one log is generated from the Logcollector configuration specified above:
...
Go to **Threat Hunting** module on the Cyb3rhq dashboard to visualize the generated alert.
