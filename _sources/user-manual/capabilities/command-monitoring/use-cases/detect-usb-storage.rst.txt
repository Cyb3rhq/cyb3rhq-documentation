...
.. meta::
    :description: Discover how to use Cyb3rhq command monitoring capability to detect USB storage device connections on Windows endpoints.

Detect USB Storage
==================

In this use case, we configured the Cyb3rhq agent to detect when a USB storage device is connected to a Windows endpoint. Then we create rules to generate alerts when the USB device is detected.
...
Go to **Threat Hunting** module on the Cyb3rhq dashboard to visualize the generated alert when a USB device is connected to the Windows endpoint.
