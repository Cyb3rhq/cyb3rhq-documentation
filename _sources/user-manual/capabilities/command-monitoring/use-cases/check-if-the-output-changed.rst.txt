...
For this endpoint, we configure Cyb3rhq to monitor the output of the Linux ``netstat`` command and alert when a change is detected.
...
Cyb3rhq has an out-of-the-box rule with ID ``533`` that generates an alert when there is a change in the ``netstat`` listening ports. The rule is defined below and is found in the `Cyb3rhq GitHub repository <https://github.com/cyb3rhq/cyb3rhq-ruleset/blob/master/rules/0015-ossec_rules.xml>`__.
...
Go to **Threat Hunting** module on the Cyb3rhq dashboard to visualize the alert showing the changes in the network.
