...
.. meta::
  :description: Follow a use case to see how Cyb3rhq runs SCA checks and triggers alerts in this section of the documentation.
...
- A Cyb3rhq server: Follow the instructions in our :doc:`quickstart guide </quickstart>` to set up a Cyb3rhq server.
- Monitored endpoint(s): Deploy any or all of the following endpoints, install a :doc:`Cyb3rhq agent </installation-guide/cyb3rhq-agent/index>` on each of them and enroll them to the Cyb3rhq server.
...
In this use case, we demonstrate how you can configure Cyb3rhq SCA to detect the presence of a keyword in a file. We monitor a file ``testfile.txt`` for the phrase ``password_enabled: yes``. The Cyb3rhq SCA module triggers an alert when it detects the pattern in the file.

Ubuntu endpoint
^^^^^^^^^^^^^^^

Take the following steps on your Ubuntu endpoint to create the file ``/usr/share/testfile.txt`` and monitor it with the Cyb3rhq SCA module:
...
#. Create a new SCA policy file ``/var/ossec/etc/custom-sca-files/keywordcheck.yml`` and add the following content to it:
   ...
   .. code-block:: YAML
      ...
      policy:
        ...
        references:
          - https://cyb3rhq.github.io/documentation/current/user-manual/capabilities/sec-config-assessment/index.html
          - https://cyb3rhq.github.io/documentation/current/user-manual/capabilities/sec-config-assessment/creating-custom-policies.html
...
#. Change the ownership of the file so Cyb3rhq has permission to it:

   .. code-block:: console
   
      # chown cyb3rhq:cyb3rhq /var/ossec/etc/custom-sca-files/keywordcheck.yml

#. Enable the policy file by adding the following lines to the ``<ossec_config>`` block of the Cyb3rhq agent configuration file at ``/var/ossec/etc/ossec.conf``:
...
#. Restart the Cyb3rhq agent to apply the changes and to run the new SCA check:

   .. code-block:: console
   
      # systemctl restart cyb3rhq-agent

#. On your Cyb3rhq dashboard, navigate to the **Configuration Assessment** module and select the Ubuntu endpoint to view the results of the custom SCA check you have created.
...
Take the following steps on your Ubuntu endpoint to create the file ``C:\Program Files\testfile.txt`` and monitor it with the Cyb3rhq SCA module:
...
#. On your Cyb3rhq dashboard, navigate to the **Configuration Assessment** module and select the Windows endpoint to view the results of the custom SCA check you have created.
...
Take the following steps on your macOS endpoint to create the file ``/usr/local/testfile.txt`` and monitor it with the Cyb3rhq SCA module:
...
#. Enable the policy file by adding the following lines to the ``<ossec_config>`` block of the Cyb3rhq agent configuration file at ``/Library/Ossec/etc/ossec.conf``:
...
#. On your Cyb3rhq dashboard, navigate to the **Configuration Assessment** module and select the macOS endpoint to view the results of the custom SCA check you have created.
...
#. Change the ownership of the file so Cyb3rhq has permission to it:
   ...
   .. code-block:: console
      ...
      # chown cyb3rhq:cyb3rhq /var/ossec/etc/custom-sca-files/processcheck.yml
...
#. Open a second command prompt and run the following command to spawn a hidden PowerShell process. This is a dummy Powershell process that sleeps for 300 seconds (5 minutes), enough time for you to restart the Cyb3rhq agent for the SCA scan to run.
...
#. Run the following commands on CMD as an Administrator to restart the Cyb3rhq agent:
