...
.. meta::
  :description: Learn more about how the Security Configuration Assessment capability of Cyb3rhq works in this section of the documentation. 
  
How SCA works
=============

Each Cyb3rhq agent has its own local database where it stores the current state of each SCA check. The Cyb3rhq server maintains an SCA database for all agents that are enrolled to it. Cyb3rhq agents only send the differences detected between scans to the Cyb3rhq server. If there has been no change, only a summary of the SCA scan is sent, thus avoiding unnecessary network traffic while keeping the SCA database on the Cyb3rhq server up to date. The Cyb3rhq server then uses those updates to issue alerts that are shown in the Cyb3rhq dashboard.
...
SCA scan results appear as alerts with SCA scan data whenever a particular check changes its status between scans. Moreover, Cyb3rhq agents only send those events necessary to keep the global status of the scan updated, avoiding potential events flooding.
...
You can view the scan summaries on the **Configuration Assessment** module on the Cyb3rhq dashboard.
...
The above SCA scan result is **Failed** because the rule did not find ``Chain INPUT * policy DROP``, ``Chain FORWARD * policy DROP``, and ``Chain OUTPUT * policy DROP`` in the output of the command ``ip6tables -L``. The steps below show how we implement the remediation steps suggested by Cyb3rhq to harden the endpoint:
...
#. Restart the Cyb3rhq agent to trigger a new SCA scan:
...
Cyb3rhq uses two integrity mechanisms  to ensure integrity between agent-side and server-side SCA states. One of the integrity mechanisms ensures the integrity of the policy files and the second ensures the integrity of scan results.
...
#. A similar message appears in the Cyb3rhq server log file ``/var/ossec/logs/ossec.log``:

   .. code-block:: none
      :class: output

      2022/11/01 15:31:23 cyb3rhq-analysisd: INFO: Policy 'cis_debian10' information for agent '001' is outdated. Requested latest scan results.

   The log shows the SCA policy file and the affected Cyb3rhq agent.

#. The Cyb3rhq server flushes its stored data for that SCA policy.
#. The Cyb3rhq agent sends the new scan results of the SCA policy.
#. The Cyb3rhq server updates its database and fires alerts for the new scan results.
...
The table below shows an example of SCA state stored in the Cyb3rhq agent and Cyb3rhq server databases.

.. table:: States stored in the Cyb3rhq agent and Cyb3rhq server databases
    :widths: auto
...
For those databases, the corresponding SHA256 hashes are:
 ...
 .. code-block:: none
    ...
    Cyb3rhq agent:   1642AB1DC478052AC3556B5E700CD82ADB69728008301882B9CBEE0696FF2C84
    Cyb3rhq server: B43037CA28D95A69B6F9E03FCD826D2B253A6BB1B6AD28C4AE57A3A766ACE610

Given that the two hashes do not match, the Cyb3rhq server requests the latest scan data from the Cyb3rhq agent and refreshes its database with the newly received status information.