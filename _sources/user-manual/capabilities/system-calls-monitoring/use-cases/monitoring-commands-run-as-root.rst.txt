...
In this use case, we use Cyb3rhq and Linux Audit system to monitor all commands executed using root privileges. 
...
#. Add the rules below in the ``/etc/audit/audit.rules`` audit rule file:
   ...
   .. code-block:: console
      ...
      # echo "-a exit,always -F euid=0 -F arch=b64 -S execve -k audit-cyb3rhq-c" >> /etc/audit/audit.rules
      # echo "-a exit,always -F euid=0 -F arch=b32 -S execve -k audit-cyb3rhq-c" >> /etc/audit/audit.rules
   ...
   - ``-k audit-cyb3rhq-c``: Specifies a key value that will be used to tag the generated audit events.
   
   Overall, this rule generates audit events for every execution of the ``execve`` system call by the root user (euid=0) and tags those events with the key ``"audit-cyb3rhq-c"``.
...
#. Reload the audit rules to apply the changes:
   ...
   .. code-block:: console   
      ...
      -a always,exit -F arch=b64 -S execve -F euid=0 -F key=audit-cyb3rhq-c
      -a always,exit -F arch=b32 -S execve -F euid=0 -F key=audit-cyb3rhq-c
...
You can visualize the alerts for ``rule.id:80792`` in the Cyb3rhq dashboard.
