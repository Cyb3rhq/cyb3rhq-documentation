...
This use case shows how Cyb3rhq can audit access to sensitive data in users' home directories. We use an audit rule that monitors all files and subdirectories in a user's home directory and tracks when another user tries to access them. Then, we create a Cyb3rhq custom rule to alert for such behavior so that security teams can take action accordingly.
...
#. Edit the audit rule file ``/etc/audit/audit.rules`` and add the following configuration:
   ...
   - ``-k power_abuse`` provides a unique ID that Cyb3rhq uses to analyze the audit logs.
...
You can visualize the alerts for ``rule.id:100210`` in the Cyb3rhq dashboard.
