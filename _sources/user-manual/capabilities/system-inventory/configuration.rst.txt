...
.. meta::
  :description: Learn more about the Syscollector configuration in this section of the Cyb3rhq documentation.

Configuration
=============

The Syscollector module is enabled by default on all endpoints where the Cyb3rhq agent is installed. You can find the Syscollector configuration in the Cyb3rhq agent configuration file at:
...
You can also use the :doc:`centralized configuration </user-manual/reference/centralized-configuration>` file to make changes to the Syscollector module across multiple monitored endpoints that belong to the same agent group. For example, the ``default`` group uses the configuration file, which you can find at ``/var/ossec/etc/shared/default/agent.conf`` on the Cyb3rhq server. Any setting done with the centralized configuration will take precedence over the local agent configuration.

The block below is the default Syscollector configuration present in the Cyb3rhq agent configuration file:
...
- ``<scan_on_start>`` initiates a system scan immediately after you restart the Cyb3rhq service on the endpoint. The default value is ``yes``. The allowed values are ``yes`` and ``no``.
