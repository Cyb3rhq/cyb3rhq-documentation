...
.. meta::
  :description: Learn about using Syscollector information to trigger alerts in this section of the Cyb3rhq documentation.

Using Syscollector information to trigger alerts
================================================

.. Note::
   This feature has been enabled again in Cyb3rhq 4.4.

You can create rules to trigger alerts based on the information collected by the Syscollector module and show the information in the alerts' description. To do this, set the ``<decoded_as>`` field as ``syscollector`` in a rule declaration. Syscollector uses the event ID ``221`` and the rule level is ``0``, so by default, it is not shown on the Cyb3rhq dashboard. Using the tag ``<if_sid>221</if_sid>`` in a custom rule, you can create rules whose alerts the Cyb3rhq dashboard will show.
...
The alert for a port opening operation is displayed in the Cyb3rhq dashboard as follows:
...
New searchable fields on the Cyb3rhq dashboard
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

You can search for any Syscollector field on the Cyb3rhq dashboard. The Cyb3rhq indexer saves the Syscollector fields as ``data.type.value``. For example, for hardware type, the ``cpu_name`` field is ``data.hardware.cpu_name``. The table lists all searchable fields for the different Syscollector properties.
...
To see Syscollector searchable fields on the Cyb3rhq dashboard, you must either:
...
After adding the configuration, restart the Cyb3rhq manager.
