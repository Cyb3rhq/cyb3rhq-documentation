...
2. Edit the Cyb3rhq agent ``/var/ossec/etc/ossec.conf`` configuration file and add the ``user_details.txt`` file for monitoring:
...
3. Restart the Cyb3rhq agent to apply the configuration:
...
Navigate to **File Integrity Monitoring** on the Cyb3rhq dashboard to view the alert generated when the FIM module detects changes to the ``user_details.txt`` file.
