...
.. meta::
  :description: Cyb3rhq includes out-of-the-box rules that trigger alerts on the creation, modification, or deletion of monitored files. Learn more about it in this section. 
  
Creating custom FIM rules
=========================

Cyb3rhq includes out-of-the-box rules that trigger alerts on the creation, modification, or deletion of monitored files. The image below shows alerts for file addition, modification, and deletion.
...
You can use custom Cyb3rhq FIM rules to monitor changes to files and directories based on specific criteria such as filename, permissions, and content. For example, you can create a custom rule to detect changes to a critical system file or configuration file. Whenever a user or process modifies the file, Cyb3rhq triggers a specific alert indicating the change and the details of the modification.

Creating custom FIM rules can extend the out-of-the-box detection capability of the Cyb3rhq FIM module. This makes it easier to identify and respond to security incidents such as data breaches, insider threats, and other cyberattacks that involve file manipulation or modification. 

This section shows you how to use the fields decoded from FIM events in custom rules. It explains what the decoded FIM events fields represent in the Cyb3rhq alert fields.

Mapping FIM fields to Cyb3rhq alerts
----------------------------------

Fields are information that the Cyb3rhq decoder extracts  from events the Cyb3rhq server receives.  Each event type has specific fields. The decoder identifies them with a field name. The Cyb3rhq server translates these fields into alert fields  and sends them to the Cyb3rhq indexer for storage. 
...
In the following examples, we demonstrate how you can customize the default FIM rules of Cyb3rhq. You can see how to create custom rules with the decoded FIM fields and what their equivalent Cyb3rhq alert fields are.
...
Cyb3rhq already has an out-of-the-box rule that generates an alert when a file permission is modified. However, in this example, you can see how to create a custom FIM rule to further customize this alert.  
...
#. Restart the Cyb3rhq server to apply the configuration changes:
...
Perform the following steps to configure the Cyb3rhq FIM module to monitor the ``/specialdir3`` directory.
...
#. Edit the Cyb3rhq agent ``/var/ossec/etc/ossec.conf`` configuration file and add the directory for monitoring:
...
Navigate to **File Integrity Monitoring** on the Cyb3rhq dashboard to view the alert generated when the FIM module detects the addition of the execute permission.
...
You can see the alert fields that correspond to the decoded FIM fields in the alert data below:
   ...
   .. code-block:: json
      ...
      {
        "_index": "cyb3rhq-alerts-4.x-2023.03.02",
        "_type": "_doc",
...
Cyb3rhq has an out-of-the-box rule that generates an alert when a monitored file is deleted or when a file in a monitored directory is deleted. In this example, we create a custom FIM rule that triggers an alert indicating the user and the application that deleted the file.
...
Perform the following steps to configure the Cyb3rhq FIM module to monitor file deletion in the ``C:\test`` directory.
...
#. Edit the Cyb3rhq agent ``C:\Program Files (x86)\ossec-agent\ossec.conf`` configuration file of the Cyb3rhq agent. Add the ``C:\test`` directory for monitoring:
...
#. Restart the Cyb3rhq agent using Powershell with administrator privilege to apply the changes:
...
Navigate to **File Integrity Monitoring** on the Cyb3rhq dashboard to view the alert generated when the FIM module detects the deletion of files in the monitored directory.
...
You can see the alert fields that correspond to the decoded FIM fields in the alert data below:
   ...
   .. code-block:: json
      ...
      {
        "_index": "cyb3rhq-alerts-4.x-2023.02.13",
        "_type": "_doc",
        ...
        "_source": {
          ...
          "rule": {
            ...
            "mail": false,
            "level": 8,
            "description": "The user \"cyb3rhq\" deleted a monitored file with  File Explorer",
            "groups": [
              "syscheck"
            ],
            "mitre": {
...
#. Create a file ``fim_alert.xml`` in the ``/var/ossec/etc/rules/`` directory on the Cyb3rhq server:
...
#. Edit the Cyb3rhq agent ``/Library/Ossec/etc/ossec.conf`` configuration file and add the ``customer_details.rtf`` file for monitoring: 
...
Navigate to **File Integrity Monitoring** on the Cyb3rhq dashboard to view the alert. In this example, you can see an alert with a severity of 12 on the Cyb3rhq dashboard when the FIM module detects changes in the monitored file.
