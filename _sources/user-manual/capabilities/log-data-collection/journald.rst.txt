...
.. meta::
   :description: The Cyb3rhq server can collect logs from the journald service. Check out this section of the documentation to learn more.
...
Cyb3rhq :doc:`Logcollector <how-it-works>` capability collects logs from sources such as kernel messages, applications, and system services using the journald service. This enables the Cyb3rhq :doc:`agentd </user-manual/reference/daemons/cyb3rhq-agentd>` daemon to forward the collected logs to the Cyb3rhq server. 

In this document, we cover the basic configuration needed to forward logs from the journald service to the Cyb3rhq server. Additionally, we show real world use cases that demonstrate the value of journald log collection.
...
Cyb3rhq collects log data from journald service in syslog format for better compatibility with the Cyb3rhq ruleset, using the following fields: 
...
Cyb3rhq uses the above fields to construct log data:

.. code-block:: none

   $TIMESTAMP $_HOSTNAME $SYSLOG_IDENTIFIER[$PID]: $MESSAGE

Once the Cyb3rhq agent collects logs using journald service, they are forwarded to the Cyb3rhq server for further processing. The Cyb3rhq analysis engine integrated within the Cyb3rhq server then performs real time analysis of the collected journald log messages, extracting relevant information from the logs and mapping them to appropriate fields using decoders. After decoding, it compares the log messages against its rules, triggering alerts when specific criteria are met, and records all alerts in both ``/var/ossec/logs/alerts/alerts.log`` and ``/var/ossec/logs/alerts/alerts.json`` files on the Cyb3rhq server.

The image below illustrates the flow of log data collection from the journald service and analysis in Cyb3rhq.
...
The Cyb3rhq agent allows you to configure the local :doc:`ossec.conf </user-manual/reference/ossec-conf/index>` configuration file on the monitored endpoint to collect journald log messages. This configuration is enabled by default in the ``/var/ossec/etc/ossec.conf`` file, as shown below:
...
The above configuration in the Cyb3rhq agent forwards all logs stored in the journald service to the Cyb3rhq server. 
...
It is worth noting that Cyb3rhq only collects future log events from the journald service after a successful configuration. However, you can disable the ``only-future-events`` option if you also need to collect historical events generated since the Cyb3rhq log collector service was stopped.

Use cases
---------

Using the journald service to forward SSH and CRON logs to Cyb3rhq
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

In this section, we configure the Cyb3rhq Logcollector module on a monitored Ubuntu endpoint to capture and forward SSH and CRON log messages from the journald service to the Cyb3rhq server. Then we utilize custom decoders and rules on the Cyb3rhq server to process the received log messages and visualize the alerts.

Ubuntu
~~~~~~

Below, we show Cyb3rhq agent configurations for collecting SSH and CRON logs from the journald service on an Ubuntu endpoint.
...
#. Modify and add the following ``<localfile>`` configuration block to the ``<ossec_conf>`` section in the ``/var/ossec/etc/ossec.conf`` file:
   ...
   .. note::
      ...
      When a log collected from the journald service matches two or more filters in a ``<localfile>`` block, the Cyb3rhq agent sends the log only once. This avoids sending duplicate logs.
...
Cyb3rhq decodes SSH logs and generates alerts from them by default. We create custom decoders and rules for the CRON logs to extract relevant data from them and generate security alerts. 

Perform the steps below on your Cyb3rhq server.
...
#. Run the following command from any endpoint to establish an SSH connection to the monitored Ubuntu endpoint:
   ...
   .. code-block:: console
      ...
      # ssh <USER_NAME>@<CYB3RHQ_AGENT_IP_ADDRESS>

   Where:

   -  ``<USER_NAME>`` is the monitored Ubuntu endpoint user.
   -  ``<CYB3RHQ_AGENT_IP_ADDRESS>`` is the IP address of the monitored Ubuntu endpoint.
...
The image below displays a security alert generated on the Cyb3rhq dashboard when an SSH connection is established to the Ubuntu endpoint.
...
The image below displays a security alert generated on the Cyb3rhq dashboard when a CRON service executed ``hello.py`` on the Ubuntu endpoint.
...
Using the journald service to forward Docker logs to Cyb3rhq 
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

In this section, we configure the journald service on a monitored Ubuntu endpoint to capture and forward Docker activity log messages to the Cyb3rhq server. Then we utilize custom decoders and rules on the Cyb3rhq server to process the received log messages and visualize the alerts.

Ubuntu
~~~~~~

Perform the steps below on your monitored Ubuntu endpoint to forward Docker log messages to the Cyb3rhq server.

#. Modify and add the following line to the Cyb3rhq agent configuration file ``/var/ossec/etc/ossec.conf``:
...
The image below displays a security alert generated on the Cyb3rhq dashboard when Docker started the container with ID ``ac64e69504d9``.
...
The image below displays a security alert generated on the Cyb3rhq dashboard when the Docker service stops a container with container ID ``ac64e69504d9``.
