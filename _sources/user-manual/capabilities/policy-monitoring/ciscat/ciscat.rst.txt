...
.. meta::
  :description: The CIS-CAT wodle has been developed in order to integrate CIS benchmark assessments into Cyb3rhq agents. Learn more about it. 
...
The **CIS-CAT wodle** has been developed for the purpose of integrating CIS benchmark assessments into Cyb3rhq agents.
...
The CIS-CAT Cyb3rhq module integrates CIS benchmark assessments into Cyb3rhq agents and reports the results of each scan in the form of an alert.
...
1. In the configuration file, ``ossec.conf``, set up a section as follows:
    ...
    Make sure the paths are correct for the location of your Java and the CIS-CAT tool. For both cases, you could specify the full path, or a relative path to the Cyb3rhq installation folder. Also, consider the following tips when configuring the ``content`` section:
...
2. After restarting the Cyb3rhq agent, the benchmark checks will be executed at the specified interval, triggering alerts as shown below.
...
Since Cyb3rhq v3.5.0, the report summary is stored in the agent DB to query it by the Cyb3rhq API. This allows knowing about the last scan every time the user wants to.
