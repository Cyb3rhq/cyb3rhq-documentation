...
.. meta::
   :description: Find use cases on monitoring the Docker environment with Cyb3rhq in this section of our documentation.

Use cases
=========

In these use cases, we create a Docker setup, and demonstrate how Cyb3rhq logs activities that occur in the Docker environment.
...
-  :doc:`Install Cyb3rhq agent </installation-guide/cyb3rhq-agent/index>` on the Docker server, and enroll it to a Cyb3rhq server.
-  :ref:`Enable the Cyb3rhq Docker listener <enable-cyb3rhq-docker-listener>` on the Docker server.
...
In this use case, we demonstrate how Cyb3rhq monitors Docker events and triggers alerts for interaction with containers and images. Perform the following steps to test the use case:
...
All Docker container runtime logs on Linux endpoints are saved by default to ``/var/lib/docker/containers/<CONTAINER_ID>/<CONTAINER_ID>-json.log``. In this use case, we show how Cyb3rhq monitors the runtime logs of a web container to detect web attacks.

Cyb3rhq agent configuration
^^^^^^^^^^^^^^^^^^^^^^^^^

#. Configure the Cyb3rhq agent on the Docker server to forward all container runtime logs to the Cyb3rhq server. Add the following configuration to the ``/var/ossec/etc/ossec.conf`` agent configuration file:
   ...
   -  We use wildcards ``*`` in the ``<location>`` field of the configuration so Cyb3rhq can monitor the dynamically named log file path. The file path of the container log usually contains the container ID as shown in the pattern ``/var/lib/docker/containers/<CONTAINER_ID>/<CONTAINER_ID>-json.log``
...
#. Add the following decoders to the ``/var/ossec/etc/decoders/local_decoder.xml`` decoder file on the Cyb3rhq server:
   ...
   -  The ``web-accesslog-docker`` decoder extracts the relevant fields in a web log, and sets the log type to ``web-log`` so the Cyb3rhq analysis engine can analyze the log for web attacks. 
   -  The ``json`` decoder enables Cyb3rhq to continue decoding the log as a regular json log in case it does not meet the criteria of the first decoder ``web-accesslog-docker``.
...
#. Run the following command on the Cyb3rhq server to simulate an SQL injection (SQLi) attack. Replace ``<WEB_IP_ADDRESS>`` with the IP address of the Docker server:
...
Navigate to the **Threat Hunting** module on the Cyb3rhq dashboard to view the alert. The following alert is generated when we simulate the SQLi attack.
