...
.. meta::
  :description: The Cyb3rhq GCP Pub/Sub module allows you to pull log data from Google Pub/Sub. Learn more about how to configure the module in this section.
...
.. note::
   ...
   The Cyb3rhq GCP module supports only one ``gcp-pubsub`` section per :ref:`Cyb3rhq configuration file <reference_ossec_conf>`. To configure more than one service, you need to deploy multiple agents.
...
For example ``<project_id>cyb3rhq-dev</project_id>``.
...
For example ``<subscription_name>cyb3rhq-name</subscription_name>``.

credentials_file
^^^^^^^^^^^^^^^^

Path to the Google Cloud credentials file. It can be absolute path or relative to ``CYB3RHQ_HOME``.
...
This option has no effect. The module now uses the :ref:`cyb3rhq_modules.debug <cyb3rhq_modules_options>` level to set its logging level.
...
Pull logs on Cyb3rhq agent start or restart.
...
.. code-block:: xml
    ...
    <gcp-pubsub>
        ...
        <project_id>cyb3rhq-dev</project_id>
        <subscription_name>cyb3rhqdns</subscription_name>
