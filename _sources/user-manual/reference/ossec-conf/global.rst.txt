...
.. meta::
  :description: The ossec.conf file is the main configuration file on the Cyb3rhq manager and plays an important role on the agents. Learn more about the global configuration here.
...
This option defines how the Cyb3rhq server identifies itself when sending mail.
...
.. warning::
   ...
   Disabling ``jsonout_output`` disrupts the alerts indexing process. In a default installation, Filebeat reads the ``alerts.json`` file and sends this information to the Cyb3rhq indexer. If the writing to this file stops, alerts are no longer indexed.
...
This setting toggles whether to query the external Cyb3rhq Cyber Threat Intelligence (CTI) service for any available Cyb3rhq updates.
