...
.. meta::
  :description: Learn how to remotely configure agents using agent.conf. In this section of the Cyb3rhq documentation, you will find which capabilities can be configured remotely.
...
For example, for the ``group1`` group, it is in ``/var/ossec/etc/shared/group1``.  Each of these files should be readable by the ``cyb3rhq`` user.
...
.. topic:: Example
   ...
   To get the agent name and operating system information, you can run the ``agent_control`` utility.
    ...
    .. code-block:: none
        ...
        Cyb3rhq agent_control. Agent information:
        Agent ID:   001
        Agent Name: agent01
        IP address: any
        Status:     Active
...
4. Confirm that the agent received the configuration:
    ...
    The ``agent_groups`` tool or the Cyb3rhq API endpoint :api-ref:`GET /agents <operation/api.controllers.agent_controller.get_agents>` can show whether the group configuration is synchronized in the agent or not:
...
5. Restart the agent:
    ...
    .. code-block:: none
        ...
        Cyb3rhq agent_control: Restarting agent: 1032
...
The Cyb3rhq manager has the capability to download configuration files like ``merged.mg`` as well as other files to be merged for the groups that you want.
