...
.. meta::
   :description: Cyb3rhq can be integrated with several Identity Providers (IdP) to implement Single Sign-On (SSO). Learn how to setup SSO with read-only role in this section of the Cyb3rhq documentation.

Setup single sign-on with read-only role
========================================

This section describes how Cyb3rhq can be integrated with several Identity Providers (IdP) to implement Single Sign-On (SSO) with a read-only role.

The guide assumes you already have a Cyb3rhq indexer and a Cyb3rhq dashboard installation. To learn more, see the :doc:`installation guide </installation-guide/index>`.

Required parameters
-------------------

The following parameters are required to make the configurations on the Cyb3rhq dashboard instance:
...
-  ``kibana_url``: URL to access the Cyb3rhq dashboard.
...
.. note::
   ...
   -  OpenSearch and the SAML assertion are case sensitive. Therefore the values on the IDP and in the SAML configuration of the Cyb3rhq indexer have to be exactly the same.
