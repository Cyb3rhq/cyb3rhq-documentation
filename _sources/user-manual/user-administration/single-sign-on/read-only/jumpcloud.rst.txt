...
.. meta::
   :description: Jumpcloud is a Unified Device and Identity Access Management platform. Learn more about it and the read-only role in this section of the Cyb3rhq documentation.
...
#. Create a new group and assign the user.
   ...
   #. Go to **User Management** > **User Groups** > **(+)** and give a name to the group. In our case, this is ``cyb3rhq-readonly``.
...
#. Create a new role mapping for the backend role. Follow these steps to create a new role mapping, and grant read-only permissions to the backend role.
   ...
   #. Under **Backend roles**, add the name of the group you created in JumpCloud  and click **Map** to confirm the action. In our case, the backend role is ``cyb3rhq-readonly``.
...
#. Check the value of ``run_as`` in the ``/usr/share/cyb3rhq-dashboard/data/cyb3rhq/config/cyb3rhq.yml`` configuration file. If ``run_as`` is set to ``false``, proceed to the next step.
   ...
   #. Click **Create Role mapping** and complete the empty fields with the following parameters:
      ...
      - **Value**: Assign the name of the group you created in JumpCloud. In our case, the backend role is ``cyb3rhq-readonly``.

      .. thumbnail:: /images/single-sign-on/jumpcloud/read-only/Cyb3rhq-role-mapping-RO.png
