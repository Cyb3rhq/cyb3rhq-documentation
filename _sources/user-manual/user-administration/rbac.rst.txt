...
.. meta::
  :description: This section of the Cyb3rhq documentation explains what a role-based access control system is and how you can use it with Cyb3rhq. 
  
.. _cyb3rhq-rbac:

Cyb3rhq RBAC - How to create and map internal users
=================================================

Cyb3rhq RBAC allows access to Cyb3rhq resources based on the roles and policies assigned to the users. It is an easy-to-use administration system that enables to manage users' or entities' permissions to the system resources. To learn more, see the :doc:`Role-Based Access Control </user-manual/api/rbac/index>` section. 

The Cyb3rhq platform includes an internal user database that can be used for authentication. It can also be used in addition to an external authentication system such as LDAP or Active Directory.  Learn how to create users and map them with Cyb3rhq in the below sections.
...
#. To map the user with Cyb3rhq, follow these steps:
   ...
   For the role mapping to take effect, make sure that ``run_as`` is set to ``true`` in the ``/usr/share/cyb3rhq-dashboard/data/cyb3rhq/config/cyb3rhq.yml`` configuration file. Restart the Cyb3rhq dashboard service and clear your browser cache and cookies.
...
#. To map the user with Cyb3rhq, follow these steps:
   ...
   #. Click **Create Role mapping** and complete the empty fields with the following parameters:
      ...
      Cyb3rhq includes an extensive list of :ref:`default policies<api_rbac_reference_default_policies>` and :ref:`roles <api_rbac_reference_default_roles>`. Additionally, you can create custom policies and roles to suit your needs. To see an example, check our :ref:`Use case: Give a user permissions to manage a group of agents <cyb3rhq-rbac-use-case-agents-group>` below. 
...
.. _cyb3rhq-rbac-use-case-agents-group:
...
This process involves adding a label in the agent group's centralized configuration to identify the Cyb3rhq alerts coming from this group of agents, creating an internal user, and giving it reading permission only for those documents that correspond to the group of authorized agents.

It also involves mapping this user with the Cyb3rhq API, creating a custom policy that includes permissions to read, restart, upgrade, among other actions over a group of agents, and finally creating a custom role and mapping it to our internal user.
...
You have now added a group label that allows us to identify all the Cyb3rhq alerts coming from this group of agents. Note that only new alerts will include this group label. 
...
You have now created an internal user and assigned it reading permissions over the Cyb3rhq alerts and Cyb3rhq monitoring documents from the authorized agents group. 

Mapping with Cyb3rhq
^^^^^^^^^^^^^^^^^^

To map the user with Cyb3rhq, follow these steps:

#. Click **☰** to open the menu on the Cyb3rhq dashboard, go to **Server management** > **Security**, and then **Policies** to open the policies page.
...
You have now created a new internal user and mapped it to manage a Cyb3rhq agents' group. Authenticate with the new user and open the Cyb3rhq dashboard, see that only ``Team_A`` agents' alerts and information are displayed.  
