...
To re-index an existing index, perform the following steps on either the Cyb3rhq dashboard or the Cyb3rhq server.
...
Run the following command on any Cyb3rhq central component that is allowed to authenticate to the Cyb3rhq API. Replace ``<INDEXER_USERNAME>`` and ``<INDEXER_PASSWORD>`` with the indexer username and password:
