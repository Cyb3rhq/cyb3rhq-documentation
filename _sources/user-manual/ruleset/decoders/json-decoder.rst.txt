...
.. meta::
   :description: Cyb3rhq incorporates a default decoder for JSON logs, enabling the extraction of data from any source in this format. Learn more in this section of the documentation.
    
JSON decoder
============

Cyb3rhq incorporates a default decoder for JSON logs, enabling the extraction of data from any source in this format.
...
The following example shows how Cyb3rhq decodes a JSON log and generates an alert for *Suricata*.
...
We can now run the ``/var/ossec/bin/cyb3rhq-logtest`` tool on the Cyb3rhq server to test the log sample and have insights into the current decoding.
...
When, testing the log sample with ``/var/ossec/bin/cyb3rhq-logtest`` we obtain the following output:
...
When, testing the log sample with ``/var/ossec/bin/cyb3rhq-logtest``, we can observe the decoded fields by the JSON decoder, as well as the matched field from the regex expression:
