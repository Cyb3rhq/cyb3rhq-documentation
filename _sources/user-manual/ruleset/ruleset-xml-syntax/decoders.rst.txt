...
.. meta::
   :description: The Cyb3rhq decoders extract the information from the received events. learn more in this section of the documentation.
...
The first step should always be to run the ``/var/ossec/bin/cyb3rhq-logtest`` utility and  input the event log to test the current decoder and rule before creating your own decoder.
...
The ``decoder`` option serves as the root element of a decoder file in Cyb3rhq. It encapsulates the definition of a decoder, including its name, type, and the specific attributes that dictate how it processes and extracts information from log messages.
...
Allows Cyb3rhq to track events over multiple log messages based on a decoded id. This is particularly useful for logs that span multiple lines or entries.
