...
.. meta::
   :description: The Cyb3rhq Ruleset analyzes incoming events and generates alerts. Learn more about Rules syntax in this section.

Rules Syntax
============

The Cyb3rhq ruleset, combined with any custom rules, analyzes incoming events. It generates alerts when all specified conditions within a rule are met. The ruleset is constantly expanding and improving thanks to the collaborative efforts of our developers and growing community.
...
Groups categorize alerts. They allow filtering related alerts in the Cyb3rhq dashboard.

The default Cyb3rhq ruleset already includes rules that use groups like ``syscheck,``, ``attack,``, and ``syslog,``. As an example, you can filter alerts for these categories by querying ``rule.groups: attack`` or ``rule.groups: (syscheck OR syslog)`` in the Cyb3rhq dashboard.
...
.. code-block:: xml
   ...
   <rule id="140101" level="12">
     ...
     <user negate="yes">cyb3rhq|root</user>
     <description>Unexpected user successfully logged to the system.</description>
   </rule>

This rule triggers when a user different from ``root`` or ``cyb3rhq`` successfully logs in to the system.
...
+----------------------+------------------------+
| Docker integration   | Cyb3rhq-Docker           |
+----------------------+------------------------+
...
Since Cyb3rhq version 3.3, it is possible to include any decoded field (static or dynamic) to the description message. You can use the following syntax: ``$(field_name)`` to add a field to the description.
...
| Default Value      | n/a                                                                                                                                   |
+--------------------+---------------------------------------------------------------------------------------------------------------------------------------+
| Allowed values     | Path to the CDB file to be used for lookup from the Cyb3rhq directory. Must also be included in the ``/var/ossec/etc/ossec.conf`` file. |
+--------------------+---------------------------------------------------------------------------------------------------------------------------------------+
