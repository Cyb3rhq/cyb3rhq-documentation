...
.. meta::
  :description: CBD lists are used to create a white/black list of users, file hashes, IPs, or domain names. Learn more about how to create CBD lists with Cyb3rhq. 
  
Using CDB lists
===============

Cyb3rhq is able to check if a field extracted during the decoding phase is in a CDB list (constant database). The main use case of this feature is to create a white/black list of users, file hashes, IP addresses, or domain names.
...
CDB lists are built and loaded automatically when the Cyb3rhq analysis engine starts. Therefore, you need to restart the Cyb3rhq manager service when you add or modify a CDB list.

Adding the list in the Cyb3rhq server configuration file
------------------------------------------------------

Perform the following steps to add a CDB list to the Cyb3rhq server configuration file.
...
#. Restart the Cyb3rhq manager via the CLI to apply the changes:
