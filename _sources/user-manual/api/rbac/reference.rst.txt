...
+--------------------+---------------------------------------------------------+--------------------------------------+
| decoder:file       | Reference decoder file via its filename                 | decoder:file:0005-cyb3rhq_decoders.xml |
+--------------------+---------------------------------------------------------+--------------------------------------+
...
The :api-ref:`/active-response <tag/Active-response>` endpoint of the Cyb3rhq server API allows users to interact with the Cyb3rhq active response module.
...
The :api-ref:`/agents <tag/Agents>` endpoint of the Cyb3rhq server API allows users to enroll and manage agents on the Cyb3rhq server.
...
The :api-ref:`/ciscat <tag/Ciscat>` endpoint of the Cyb3rhq server API enables users to retrieve specific information from the results of CIS-CAT scans carried out on the Cyb3rhq agents.
...
The :api-ref:`/cluster <tag/Cluster>` endpoint of the Cyb3rhq server API allows users to manage the configuration and health of the master node and the worker nodes in the Cyb3rhq cluster.
...
The :api-ref:`/decoder <tag/Decoders>` endpoint of the Cyb3rhq server API enables users to manage and retrieve information about the decoders included in the Cyb3rhq server.
...
The :api-ref:`/event <tag/Events>` endpoint of the Cyb3rhq server API allows users to ingest security events to the Cyb3rhq analysis engine.
...
The :api-ref:`/groups <tag/Groups>` endpoint of the Cyb3rhq server API enables users to group Cyb3rhq agents into distinct subsets for centralized configurations.
...
The :api-ref:`/lists <tag/Lists>` endpoint of the Cyb3rhq server API allows users to retrieve and manage the CDB lists that are used for checking malicious files on Cyb3rhq agents.
...
The :api-ref:`/logtest <tag/Logtest>` endpoint of the Cyb3rhq server API allows users to test and verify new rules and decoders against provided log examples in the Cyb3rhq analysis engine.
...
The :api-ref:`/manager <tag/Manager>` endpoint of the Cyb3rhq server API enables users to manage and collect relevant information from the Cyb3rhq manager.
...
The :api-ref:`/mitre <tag/MITRE>` endpoint of the Cyb3rhq server API allows users to retrieve a high-level overview of the corresponding tactics and techniques from the MITRE ATT&CK database.
...
The :api-ref:`/rootcheck <tag/Rootcheck>` endpoint of the Cyb3rhq server API enables users to interact with the Cyb3rhq rootcheck module and retrieve results from the scans on the Cyb3rhq agents.
...
The :api-ref:`/rules <tag/Rules>` endpoint of the Cyb3rhq server API lets users manage and retrieve information about the Cyb3rhq rules that are used to analyze incoming events and generate alerts.
...
The :api-ref:`/sca <tag/SCA>` endpoint of the Cyb3rhq server API allows users to interact with the Cyb3rhq SCA module and collect relevant SCA scan results from Cyb3rhq agents.
...
The :api-ref:`/security <tag/Security>` endpoint of the Cyb3rhq server API enables administrators to manage security-related aspects within the Cyb3rhq environment.
...
The :api-ref:`/syscheck <tag/Syscheck>` endpoint of the Cyb3rhq server API allows users to interact with the Cyb3rhq File Integrity Monitoring module as it initiates routine scans and retrieves syscheck results.
...
The :api-ref:`/syscollector <tag/Syscollector>` endpoint of the Cyb3rhq server API allows users to collect system information from monitored endpoints and send them to the Cyb3rhq server.
...
The :api-ref:`/tasks <tag/Tasks>` endpoint of the Cyb3rhq server API enables users to get status information about the tasks performed by the Cyb3rhq manager.
...
The :api-ref:`/vulnerability <tag/Vulnerability>` endpoint of the Cyb3rhq server API allows users to perform vulnerability detector scans and collect relevant information about vulnerabilities from Cyb3rhq agents. This API endpoint has been deprecated since version 4.7.
...
Allow sending active response commands to Cyb3rhq agents.
...
Allow managing all decoder files in the Cyb3rhq server.
...
Allow reading all decoder files in the Cyb3rhq server.
...
Allow sending events to the Cyb3rhq analysis engine.
...
Allow managing all CDB lists files on the Cyb3rhq server.
...
Allow reading the path of  all the lists in the Cyb3rhq server.
...
Allow managing all rule files in the Cyb3rhq server.
...
The administrator role has full access to all endpoints in the Cyb3rhq server API.
...
Manager administrator of the Cyb3rhq server cluster, this role has full access to all manager related functionalities.
...
.. warning::
   ...
   run_as permissions through these mapping rules can only be obtained with ``cyb3rhq-wui`` user. These rules will never match an authorization context for any other Cyb3rhq server API user.

wui_elastic_admin
^^^^^^^^^^^^^^^^^

Administrator permissions for the elastic users of the Cyb3rhq dashboard.
...
Administrator permissions for the opendistro users of the Cyb3rhq dashboard.
