...
.. meta::
   :description: This section provides several use cases to demonstrate some of the potentials of the Cyb3rhq server API.

Use cases
---------

This section provides several use cases to demonstrate some of the potentials of the Cyb3rhq server API. You can find details about all possible API requests in the :ref:`reference <api_reference>` section.
...
Analyzing the File Integrity Monitoring (FIM) database of a Cyb3rhq agent
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

You can utilize the Cyb3rhq server API to display information about all files monitored by the Cyb3rhq FIM module. The following example shows all events associated with Python ``.py`` files installed on a monitored endpoint with agent ID ``001``:
...
You can retrieve various details about the Cyb3rhq manager through the Cyb3rhq server API. These details include configuration, status, logs, and more. The following example demonstrates how to retrieve the status of each Cyb3rhq daemon:
...
.. code-block:: json
...

   {
     "data": {
       "affected_items": [
         {
           "cyb3rhq-agentlessd": "running",
           "cyb3rhq-analysisd": "running",
           "cyb3rhq-authd": "running",
           "cyb3rhq-csyslogd": "running",
           "cyb3rhq-dbd": "stopped",
           "cyb3rhq-monitord": "running",
           "cyb3rhq-execd": "running",
           "cyb3rhq-integratord": "running",
           "cyb3rhq-logcollector": "running",
           "cyb3rhq-maild": "running",
           "cyb3rhq-remoted": "running",
           "cyb3rhq-reportd": "stopped",
           "cyb3rhq-syscheckd": "running",
           "cyb3rhq-clusterd": "running",
           "cyb3rhq-modulesd": "running",
           "cyb3rhq-db": "running",
           "cyb3rhq-apid": "stopped"
         }
       ],
       "total_affected_items": 1,
       "total_failed_items": 0,
...
You can dump the Cyb3rhq manager current configuration with the request below (the response is shortened for brevity):
       ...
       "affected_items": [
         {
           "global": {
             ...
             "email_from": "cyb3rhq@test.example",
             "email_maxperhour": "12",
...
Exploring Cyb3rhq agent management
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

You can use the Cyb3rhq server API to manage the Cyb3rhq agents.
       ...
       "affected_items": [
         {
           "node_name": "worker2",
           "status": "active",
           "manager": "cyb3rhq-worker2",
           "version": "Cyb3rhq v4.7.4",
           "id": "001",
           "name": "cyb3rhq-agent1"
         },
         {
...
Add a new Cyb3rhq agent by sending an API request with the agent name and its IP address:
...
You can utilize the Cyb3rhq server API to ingest security events into the Cyb3rhq manager for analysis.
...
Conclusion
^^^^^^^^^^
In conclusion, these examples showcase the capabilities of the Cyb3rhq API. Explore the :ref:`reference <api_reference>` document to discover the full range of available Cyb3rhq server API requests.
