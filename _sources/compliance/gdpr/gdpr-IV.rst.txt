...
.. meta::
  :description: Check out this section to learn more about how to use Cyb3rhq for GDPR IV (The General Data Protection Regulation of the European Union). 
...
Using the :doc:`File Integrity Monitoring (FIM) </user-manual/capabilities/file-integrity/index>` module, Cyb3rhq ensures that certain established protection measures are met. Cyb3rhq uses the FIM module to help enhance security in the processing of data. It logs information about who modified the data, when the modification occurred, and all related events impacting the data of interest.

Use case: Detect changes to file attributes
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

In this use case, you have to configure the Cyb3rhq agent to detect changes to ``/root/personal_data`` or its attributes, as well as detect who made the changes. The configuration in this use case is specific to an Ubuntu 22.04 endpoint. Then, you need to change the owner of a file to trigger an alert.
...
Cyb3rhq supports the storage of information about file integrity monitoring and system events. It uses the :doc:`log data collection </user-manual/capabilities/log-data-collection/how-it-works>` capability to store all the events the Cyb3rhq server receives in the archives file ``/var/ossec/logs/archives/archives.log``. Additionally, the ``/var/ossec/logs/archives/alerts.log`` file stores alerts from rules triggered. These logs help in performing various activities, such as data audits and threat hunting.

Use case: Store all logs generated from an endpoint
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

In this use case, you have to set storage of all events from monitored endpoints in the Cyb3rhq archives, whether they generate an alert or not.

Cyb3rhq server
~~~~~~~~~~~~

#. Edit the Cyb3rhq server configuration file ``/var/ossec/etc/ossec.conf`` and set the ``<logall>`` option to ``yes``. We have highlighted the ``<logall>`` option in the configuration block below:
	...
	.. code-block:: xml
		...
		<global>
		  ...
		  <smtp_server>smtp.example.cyb3rhq.github.io</smtp_server>
		  <email_from>cyb3rhq@example.cyb3rhq.github.io</email_from>
...
#. Check the contents of the ``/var/ossec/logs/archives/archives.log`` file on the Cyb3rhq manager, you can see events including those that do not trigger an alert:
	...
	.. code-block:: none
		...
		2022 Sep 30 09:57:25 cyb3rhq-manager->/var/log/syslog Sep 30 09:57:25 cyb3rhq-manager multipathd[504]: sda: failed to get sgio uid: No data available
		2022 Sep 30 09:57:25 cyb3rhq-manager->/var/log/syslog Sep 30 09:57:25 cyb3rhq-manager multipathd[504]: sda: failed to get sysfs uid: No data available
		...
		2022 Sep 30 09:57:31 cyb3rhq-manager->/var/log/syslog Sep 30 09:57:30 cyb3rhq-manager multipathd[504]: sda: add missing path
		2022 Sep 30 09:57:31 cyb3rhq-manager->/var/log/syslog Sep 30 09:57:30 cyb3rhq-manager multipathd[504]: sda: failed to get sysfs uid: No data available
		2022 Sep 30 09:57:31 cyb3rhq-manager->/var/log/syslog Sep 30 09:57:30 cyb3rhq-manager multipathd[504]: sda: failed to get udev uid: Invalid argument
		2022 Sep 30 09:57:31 cyb3rhq-manager->/var/log/syslog Sep 30 09:57:30 cyb3rhq-manager multipathd[504]: sda: failed to get sgio uid: No data available
		2022 Sep 30 09:57:31 cyb3rhq-manager->/var/log/syslog Sep 30 09:57:31 cyb3rhq-manager multipathd[504]: sdb: add missing path
		2022 Sep 30 09:57:31 cyb3rhq-manager->/var/log/syslog Sep 30 09:57:31 cyb3rhq-manager multipathd[504]: sdb: failed to get udev uid: Invalid argument
...
This article requires carrying out risk assessments to find out what risks processing actions pose to personal user data. The Cyb3rhq log data analysis module and default ruleset help meet aspects of this article by monitoring actions taken by data administrators. With this, the data protection officer is able to check who is accessing and processing the data, whether they are authorized to do so, and whether they are who they say they are.

Use case: Invalid SSH login attempts
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

In this use case, there is an example of a Cyb3rhq rule to detect SSH authentication attempts with an invalid user. The Cyb3rhq server receives SSH authentication logs from the monitored endpoint. Then, the log data analysis module subsequently decodes and evaluates these logs against default Cyb3rhq rules to determine if they match the behavior of interest.
...
When an invalid login attempt triggers rule 5710, you can see the alert below on the Cyb3rhq dashboard.
...
This article requires that organizations report data breaches to the appropriate authorities within a stipulated time frame. Cyb3rhq facilitates this communication by sending email notifications when events trigger a specific alert or a group of alerts related to events about personal data. Refer to the Cyb3rhq :ref:`email alerts <configuring_email_alerts>` section of the documentation for more information on configuring email notifications.

Use case: Email alert on failed login
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

In this use case, you configure Cyb3rhq to generate an alert and send notifications to the specified email addresses whenever a user login attempt via SSH fails.

#. Edit the email section of the Cyb3rhq manager configuration file ``/var/ossec/etc/ossec.conf`` as follows to implement email notifications:
	...
	.. code-block:: xml
		...
		<ossec_config>
		  <global>
		    ...
		    <email_from>cyb3rhq@test.example</email_from>
		  </global>
		</ossec_config>
...
.. code-block:: none
	...
	From: Cyb3rhq <cyb3rhq@test.example>               5:03 PM (2 minutes ago)
	to: me
	-----------------------------
	Cyb3rhq Notification.
...
This article recommends performing a risk assessment on data processing channels and the impact of the risks identified on data protection. Cyb3rhq can support the risk assessment outcome by categorizing FIM alerts for certain files or directories and increasing the alert levels based on the risk assessment reports.
...
#. Restart the Cyb3rhq manager for the configuration changes to apply:
...
#. Create the file ``regular_data.txt`` in the ``/customers`` directory and add some content:
	...
	You can see a level 7 alert generated in the **File Integrity Monitoring** section of the Cyb3rhq dashboard to show that a file in the monitored directory was modified.
...
Cyb3rhq helps meet this article of the GDPR by providing security measures such as:
...
-  `Using constant database (CDB) lists to detect and remove malicious files <https://cyb3rhq.github.io/blog/detecting-and-responding-to-malicious-files-using-cdb-lists-and-active-response/>`__.
