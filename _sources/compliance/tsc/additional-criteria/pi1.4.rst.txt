...
.. meta::
   :description: Cyb3rhq helps meet the processing integrity PI1.4 requirement by monitoring and reporting file changes using the FIM module.
...
Use case: Detecting file changes using the Cyb3rhq File Integrity Monitoring module
---------------------------------------------------------------------------------

This use case shows how Cyb3rhq helps meet the *processing integrity PI1.4* requirement by monitoring and reporting file changes using the FIM module. In this scenario, we show how you can configure the Cyb3rhq agent on a Ubuntu 22.04 endpoint to detect changes in the ``critical_folder`` directory.
...
#. Add the highlighted configuration to the ``<syscheck>`` block of the Cyb3rhq agent configuration file ``/var/ossec/etc/ossec.conf``:
...
#. Select **TSC** from the Cyb3rhq dashboard to view the alert with rule ID ``550``.
