...
.. meta::
  :description: The Cyb3rhq module for GitHub enables the collection of audit logs from GitHub through its API. Check out this section of our documentation to learn more about it.

Monitoring GitHub audit logs
============================

Organizational administrators proactively utilize GitHub audit logs to review the actions performed by members within their organization. It includes details such as the user who performs the action, the nature of the action, and the timestamp of its execution. The Cyb3rhq module for GitHub enables the collection of audit logs from GitHub through its API. Cyb3rhq initiates an HTTP GET request to GitHub API endpoint ``/orgs/{org}/audit-log`` to collect the audit logs. For further details, you can refer to the `GitHub REST API <https://docs.github.com/en/rest>`__ documentation.

Requirements for monitoring GitHub audit logs
---------------------------------------------

You need the following requirements on GitHub to access the audit logs with Cyb3rhq.
...
Configure Cyb3rhq to pull GitHub logs
-----------------------------------

Perform the following steps to allow Cyb3rhq to monitor, collect, and analyze the GitHub audit logs. You can either configure the Cyb3rhq module for GitHub in the Cyb3rhq server or the Cyb3rhq agent.
...
#. Append the following configuration to the ``/var/ossec/etc/ossec.conf`` file on the Cyb3rhq server.
   ...
   -  ``<enabled>``: Enables the Cyb3rhq module for GitHub. The allowed values are ``yes`` and ``no``.
   -  ``<interval>``: Defines the time interval between each execution of the Cyb3rhq module for GitHub. The default value is ``10m``, and the allowed value is any positive number that contains a suffix character indicating a time unit, such as s (seconds), m (minutes), h (hours), and d (days). 
   ...
   -  ``<only_future_events>``: When set to yes, the Cyb3rhq module for GitHub collects only events generated after you start the Cyb3rhq manager. when set to ``no``, it collects previous events generated before you start the Cyb3rhq manager. The default value is ``yes``, and the allowed values are ``yes`` and ``no``.
   ...
   -  ``<api_parameters>``: This block configures the internal options in the GitHub REST API. One sub-configuration block within ``<api_parameters>`` is ``<event_type>``.
      ...
      -  ``<event_type>``: Specifies the event types Cyb3rhq should collect. The available event types are web and git events. The default value for this configuration block is ``all``, to collect both web and git events. Allowed values are ``all``, ``web``, and ``git``.

   To learn more about the configuration options, refer to the :doc:`Cyb3rhq module for GitHub </user-manual/reference/ossec-conf/github-module>` reference.
...
You can monitor multiple GitHub organizations with Cyb3rhq by specifying the organization credentials in individual ``<api_auth>`` sections. For example, the following configuration monitors two organizations named ``organization1`` and ``organization2``.
...
-  Integrate Cyb3rhq with GitHub according to the :doc:`monitoring GitHub activity </cloud-security/github/monitoring-github-activity>` documentation.
-  An Ubuntu 23.10 endpoint with curl installed.

GitHub
^^^^^^

Create a GitHub personal access token within the ``admin:org``, ``repo``, and ``delete_repo`` scopes. We will use this token with the GitHub REST API to test the use cases by performing actions on the organization that trigger alerts on Cyb3rhq. Take the following steps to create the token:
...
The image below shows the alerts generated on the Cyb3rhq dashboard after we performed the above actions on the monitored GitHub organization.
