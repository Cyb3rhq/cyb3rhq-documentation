...
.. meta::
  :description: Learn how the Cyb3rhq Azure module works in conjunction with the Microsoft Graph REST API in this section of the documentation.
...
- `Cyb3rhq configuration`_
- `Microsoft Graph use case`_

In order to know how the Cyb3rhq Azure module works in conjunction with the `Microsoft Graph REST API`, it is important to understand first what are the Microsoft Entra ID activity reports and what kind of information they provide. Cyb3rhq can process the logs from the following Microsoft Entra ID activity reports, each one of them requiring a different query to be executed:
...
In this example, the records are in ``.json`` format. The following rules are already included in Cyb3rhq which means alerts will be generated for the logs in this example.
...
Cyb3rhq dashboard visualization
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

Once the integration is running, the results will be available in the Cyb3rhq dashboard.
