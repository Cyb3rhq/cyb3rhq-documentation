...
.. meta::
  :description: Frequently asked questions about the Cyb3rhq module for Amazon. Learn more about it in this section of the documentation.
...
When the module runs it writes its output in the ``ossec.log`` file. This log file can be found in ``CYB3RHQ_PATH/logs/ossec.log`` and under **Server management** > **Logs** if using the Cyb3rhq UI.
...
- When the module is starting:
    ...
    .. code-block:: none
        ...
        2022/03/04 00:00:00 cyb3rhq-modulesd:aws-s3: INFO: Module AWS started
        2022/03/04 00:00:00 cyb3rhq-modulesd:aws-s3: INFO: Starting fetching of logs.
...
- When Scheduled scan is set:
    ...
    .. code-block:: none
        ...
        2022/03/04 00:00:00 cyb3rhq-modulesd:aws-s3: INFO: Starting fetching of logs.
        2022/03/04 00:00:00 cyb3rhq-modulesd:aws-s3: INFO: Fetching logs finished.
...
#. Add the following line to the ``CYB3RHQ_PATH/etc/local_internal_options.conf`` file, specifying the desired debug level:
...
To understand how the :ref:`reference_ossec_global_logall_json` parameter works it is necessary to learn about the flow that is followed when processing a log until the corresponding alert is displayed in the Cyb3rhq UI. It is as follows:
...
#. The analysis engine evaluates these events and compares them with the different rules available. If the event matches any of the rules an alert is generated, which is what ultimately is shown in the Cyb3rhq UI.

With this in mind, it is possible to make use of the :ref:`reference_ossec_global_logall_json` option. When this option is activated, Cyb3rhq stores into the ``CYB3RHQ_PATH/logs/archives/archives.json`` file every event sent to the analysis engine whether they tripped a rule or not. By checking this file, it is possible to determine if the AWS events are being sent to the analysis engine and therefore the module is working as expected.
...
The module does not work and the following error messages appear in the ``ossec.log``:
    ...
    .. code-block:: none
        ...
        2022/03/03 16:01:48 cyb3rhq-modulesd:aws-s3: WARNING: Bucket:  -  Returned exit code 12
        2022/03/03 16:01:48 cyb3rhq-modulesd:aws-s3: WARNING: Bucket:  -  Unable to locate credentials
...
The module does not work and the following error messages appear in the ``ossec.log``:
    ...
    .. code-block:: none
        ...
        2022/03/03 16:06:56 cyb3rhq-modulesd:aws-s3: WARNING: Bucket:  -  Returned exit code 3
        2022/03/03 16:06:56 cyb3rhq-modulesd:aws-s3: WARNING: Bucket:  -  Invalid credentials to access S3 Bucket
...
The module does not work and the following error messages appear in the ``ossec.log``:
    ...
    .. code-block:: none
        ...
        2022/03/03 15:49:34 cyb3rhq-modulesd:aws-s3: WARNING: Bucket:  -  Returned exit code 12
        2022/03/03 15:49:34 cyb3rhq-modulesd:aws-s3: WARNING: Bucket:  -  The config profile (default) could not be found
...
The module does not work and the following error messages appear in the ``ossec.log``:
    ...
    .. code-block:: none
        ...
        2022/03/03 16:16:18 cyb3rhq-modulesd:aws-s3: WARNING: Service: cloudwatchlogs  -  Returned exit code 12
        2022/03/03 16:16:18 cyb3rhq-modulesd:aws-s3: WARNING: Service: cloudwatchlogs  -  An error occurred (InvalidClientTokenId) when calling the GetCallerIdentity operation: The security token included in the request is invalid.
...
There are no AWS alerts present in the Cyb3rhq UI
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

The module is running but no alerts are displayed in the Cyb3rhq UI.
...
CloudWatch Logs integration is running but no alert is shown in the Cyb3rhq UI
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

The module is running without any error or warning messages, but no alerts from CloudWatch Logs are displayed in the Cyb3rhq UI.

**Solution**

A common scenario is that no alerts are being generated because the events are not matching any of the available rules. Take a look to the :ref:`aws_events_processed` section to learn how to check if the AWS logs are being sent to the analysis engine.

Take into account that Cyb3rhq does not provide default rules for the different logs that can be found in CloudWatch Logs, since they can have any type of format and come from any source. Because of this, if a user wants to make use of this integration to process any custom log they will most likely have to configure their own rules for them. Take a look at the :doc:`/user-manual/ruleset/rules/custom` section to learn more about this topic.
...
#. Errors in ``ossec.log``
    ...
    +-----------+-------------------------------------------------------------------+------------------------------------------------------------------------------------------------------------------------------------------------------+
    | 1         | Unknown error                                                     | Programming error. Please, open an issue in the `Cyb3rhq GitHub repository <https://github.com/cyb3rhq/cyb3rhq/issues/new/choose>`_ with the trace of the  |
    |           |                                                                   | error.                                                                                                                                               |
    +-----------+-------------------------------------------------------------------+------------------------------------------------------------------------------------------------------------------------------------------------------+
    ...
    +-----------+-------------------------------------------------------------------+------------------------------------------------------------------------------------------------------------------------------------------------------+
    | 11        | Unable to connect to Cyb3rhq                                        | Ensure that Cyb3rhq is running.                                                                                                                        |
    +-----------+-------------------------------------------------------------------+------------------------------------------------------------------------------------------------------------------------------------------------------+
    | 12        | Invalid type of bucket                                            | Check if the type of bucket is one of the :ref:`supported <amazon_supported_services>`.                                                              |
    +-----------+-------------------------------------------------------------------+------------------------------------------------------------------------------------------------------------------------------------------------------+
    | 13        | Error sending message to Cyb3rhq                                    | Make sure that Cyb3rhq is running.                                                                                                                     |
    +-----------+-------------------------------------------------------------------+------------------------------------------------------------------------------------------------------------------------------------------------------+
    ...
    +-----------+-------------------------------------------------------------------+------------------------------------------------------------------------------------------------------------------------------------------------------+
    | 17        | Invalid file key format                                           | Ensure that the file path follows the format specified in the :doc:`Cyb3rhq documentation </cloud-security/amazon/services/supported-services/index>`  |
    +-----------+-------------------------------------------------------------------+------------------------------------------------------------------------------------------------------------------------------------------------------+
