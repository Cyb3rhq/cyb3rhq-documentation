...
.. meta::
  :description: Learn about the different ways to configure your AWS credentials when monitoring AWS services with Cyb3rhq.
...
In order to make the Cyb3rhq AWS module pull log data from the different services, it's necessary to provide access credentials to connect to them. The ``.aws/credentials`` file must be placed at the correct location. You need to create the credentials file using ``root``. The root user is the one that runs *cyb3rhq-modulesd*. Thus, the correct location is ``/root/.aws/credentials``, the home directory of the root user.
...
Cyb3rhq requires a user with permissions to pull log data from the different services. The easiest way to accomplish this is by creating a new IAM user in the AWS account.
...
Depending on the service that will be monitored, the Cyb3rhq user will need a different set of permissions. The permissions required for each service are explained on the page of each service listed in the :ref:`supported services <amazon_supported_services>` section.
