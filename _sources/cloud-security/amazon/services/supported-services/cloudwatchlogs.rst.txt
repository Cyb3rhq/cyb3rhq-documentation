...
.. meta::
  :description: AWS CloudWatch Logs is a service that allows users to centralize the logs from all their systems. Learn how to use CloudWatch Logs with Cyb3rhq here.
...
AWS CloudWatch logs can be accessed by using the Cyb3rhq CloudWatch Logs integration. The AWS API allows Cyb3rhq to retrieve those logs, analyze them, and raise alerts if applicable.
...
#. Open the Cyb3rhq configuration file (``/var/ossec/etc/ossec.conf``) and add the following configuration block to enable the integration with CloudWatch Logs:
