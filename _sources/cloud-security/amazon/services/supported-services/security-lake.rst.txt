...
.. note::
   ...
   This document guides you through setting up Cyb3rhq as a subscriber to AWS Security Lake data. To configure Cyb3rhq as a source for Amazon Security Lake, refer to :doc:`Amazon Security Lake integration </integrations-guide/amazon-security-lake/index>`
...
All logs in Amazon Security Lake use the Open Cybersecurity Schema Framework (OCSF) standard for formatting. You can use the Cyb3rhq integration for Amazon Security Lake to ingest security events from AWS services.
...
Cyb3rhq periodically checks for new SQS messages, downloads new objects, converts the files from Parquet to JSON, and indexes each event into the Cyb3rhq indexer.
To set up the Cyb3rhq integration for Amazon Security Lake as a subscriber, you need to do the following:

    #. Create a subscriber in Amazon Security Lake.
    #. Set up the Cyb3rhq integration for Amazon Security Lake.
...
After completing all required AWS prerequisites, configure a subscriber for Amazon Security Lake via the AWS console. This creates the resources you need to make the Amazon Security Lake events available for consumption in your Cyb3rhq platform deployment.
...
#. Enter a descriptive name for your subscriber. For example, Cyb3rhq.
...
#. Enter a unique value in **External ID**. For example, *cyb3rhq-external-id-value*.
...
.. code-block:: xml
        ...
        <wodle name="aws-s3">
            ...
            <subscriber type="security_lake">
                ...
                <external_id>cyb3rhq-external-id-value</external_id>
...
After setting the required parameters, restart the Cyb3rhq manager to apply the changes:

.. include:: /_templates/common/restart_manager.rst

.. note::
    
    The module execution time varies depending on the number of notifications in the queue. This affects the time to display alerts on the Cyb3rhq dashboard. If the ``<interval>`` value is less than the execution time, the :ref:`Interval overtaken <interval_overtaken_message>` message appears in the ``ossec.log`` file.
...
Visualizing alerts in Cyb3rhq dashboard 
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

Once you set the configuration and restart the manager, you can visualize the Amazon Security Lake alerts in the Cyb3rhq dashboard. To do this, go to the **Threat Hunting** module. Apply the filter ``rule.groups: amazon_security_lake`` for an easier visualization.
