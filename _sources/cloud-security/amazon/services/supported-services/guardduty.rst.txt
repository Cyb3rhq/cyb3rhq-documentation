...
.. meta::
  :description: Amazon GuardDuty is a threat detection service that continuously monitors for malicious behavior. Learn how to use GuardDuty with Cyb3rhq in this section.
...
#. On the **Destination settings** select the previously created S3 bucket and add a prefix where logs will be stored. 
   ...
   If a prefix is used it must be specified under the Cyb3rhq Bucket configuration. 

    .. thumbnail:: /images/aws/guardduty-firehose-5.png
      :align: center
      :width: 70%

#. Select the **Compression for data records** you prefer. Cyb3rhq supports any kind of compression but *Snappy*:
...
#. Open the Cyb3rhq configuration file ``/var/ossec/etc/ossec.conf`` and add the following block:
...
If an instance has an open port that is receiving a brute force attack, the following alert will be shown on the Cyb3rhq dashboard. It shows information about the attacked host, the attacker, and which port is being attacked:
...
If an API call is made from an unusual network, the following alert will be shown on the Cyb3rhq dashboard. It shows the location of the unusual network, the user who did the API calls, and which API calls it did:
...
If there is any indicator of a compromised EC2 instance, an alert will be shown on the Cyb3rhq dashboard explaining what's happening. Some examples of alerts are shown below:
...
And here are the Cyb3rhq dashboard charts for EC2 events:
