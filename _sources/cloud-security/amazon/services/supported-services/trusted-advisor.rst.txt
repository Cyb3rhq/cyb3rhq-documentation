...
Create an Amazon Kinesis Data Firehose delivery stream to be able to store the Trusted Advisor logs into the desired S3 bucket so Cyb3rhq can process them.
...
#. Access the Cyb3rhq configuration in **Settings** using the Cyb3rhq UI or by manually editing the ``/var/ossec/etc/ossec.conf`` file in the host:
...
#. Add the following :doc:`AWS S3 module </user-manual/reference/ossec-conf/wodle-s3>` configuration to the file, replacing ``cyb3rhq-aws-wodle`` with the name of the S3 bucket:
...
#. Save the changes and restart Cyb3rhq to apply the changes. The service can be manually restarted using the following command outside the Cyb3rhq UI:
