...
Your environment is configured by default to send Cyb3rhq output files to archive data.


There are two Cyb3rhq output files in JSON format:

- ``/var/ossec/logs/archives/archives.json``: If  you set ``logall_json`` to ``yes``, Cyb3rhq stores all events in this file and sends it to archive data, regardless of whether they triggered an alert.
