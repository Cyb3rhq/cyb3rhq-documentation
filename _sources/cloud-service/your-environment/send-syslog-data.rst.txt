...
.. meta::
  :description: Cyb3rhq agents work on a wide range of operating systems, but if this is not possible, you can forward syslog events to your environment.  
...
Cyb3rhq agents can run on a wide range of operating systems, but when it is not possible due to software incompatibilities or business restrictions, you can forward syslog events to your environment. This is a common use case for network devices such as routers or firewalls.

Since every communication with your environment is performed through the Cyb3rhq agent, you must configure the agent to forward the syslog events. To do so, you have these options:
...
Use rsyslog on a Linux endpoint with a Cyb3rhq agent to log to a file and send those logs to the environment.
...
#. Deploy a Cyb3rhq agent on the same endpoint with rsyslog installed.
...
#. Run the commands below to restart rsyslog and the Cyb3rhq agent:
...
Use Logstash on a Windows endpoint with a Cyb3rhq agent to receive syslog, log to a file, and send those logs to the environment.
...
#. Deploy a Cyb3rhq agent on the same endpoint that has Logstash. 
   
#. Configure the Cyb3rhq agent to read the Logstash output file by adding the following configuration to the ``C:\Program Files (x86)\ossec-agent\ossec.conf`` file:
...
#. Restart the Cyb3rhq agent. If you are running PowerShell, use the following command:

   .. code-block:: console
      
      Restart-Service Cyb3rhq
