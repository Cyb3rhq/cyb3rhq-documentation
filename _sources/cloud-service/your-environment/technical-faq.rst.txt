...
.. meta::
  :description: Get answers to the most frequently asked questions about the Cyb3rhq Cloud in this technical FAQ and get the most out of the Cyb3rhq Cloud service.  
...
- `Can I send data directly to the Cyb3rhq indexer of my environment?`_

- `Can I integrate with my Single Sign-On (SSO) method (LDAP, Okta, Active Directories)?`_

- `Do I have access to Cyb3rhq API?`_

- `Do I have access to Cyb3rhq indexer API?`_
...
All the communications are performed through Cyb3rhq agents once they are registered to the environment.
  
Is it possible to change the URL to access the environment?
-----------------------------------------------------------

It is possible to get a new URL by opening a support ticket through the **Help** section on the Cyb3rhq Cloud Console, but the previous URL is also kept.
...
You can upgrade or downgrade a setting by contacting the Cyb3rhq team through the **Help** section of your Cyb3rhq Cloud Console. See also :ref:`Adjusting environment settings <cloud_settings_adjust>`.
...
If the data ingestion is exceeded, events start to queue. If the queue becomes full, Cyb3rhq discards the incoming events, which might lead to event loss. The cloud service automatically manages the queuing mechanism, ensuring optimal resource usage.

How do I get SSH access to my environment?
------------------------------------------

SSH access is not allowed for security reasons. Environments are managed from the Cyb3rhq Cloud Console and Cyb3rhq WUI.

How can I update my environment?
--------------------------------

Cyb3rhq takes care of the updates, so your environment gets the latest version of Cyb3rhq with no downtime.
 
Can I send syslog data directly to the environment?
---------------------------------------------------

No, all the communications are performed through Cyb3rhq agents once they are registered into the environment. However, you have alternative options. For more information on how to forward syslog events to your environment, see the :ref:`Forward syslog events <cloud_your_environment_send_syslog>` section.

Can I send data directly to the Cyb3rhq indexer of my environment?
----------------------------------------------------------------

No, all the communications are performed through Cyb3rhq agents.

Can I integrate with my Single Sign-On (SSO) method (LDAP, Okta, Active Directories)?
----------------------------------------------------------------------------------------

Yes, you can access the Cyb3rhq WUI of your environment through your SSO tool. To perform this action, you need to contact the Cyb3rhq Support team through the **Help** section of your Cyb3rhq Cloud Console.


Do I have access to Cyb3rhq API?
------------------------------

You have access to the **Dev tools** through your Cyb3rhq dashboard, where you can use the API. The Cyb3rhq API is not exposed, but you can contact the Cyb3rhq team through the **Help** section of your Cyb3rhq Cloud Console to allow Cyb3rhq API access from a specific IP address.


Do I have access to Cyb3rhq indexer API?
--------------------------------------

The Cyb3rhq indexer API is not accessible by default. If you want to access it, contact the Cyb3rhq team through the **Help** section of your Cyb3rhq Cloud Console to authorize the connection from a specific IP address. After authorization is granted, you have access to the ``GET`` methods of the Cyb3rhq indexer API.
