...
.. meta::
  :description: Cyb3rhq provides an automated way of building macOS packages. Learn how to build your own Cyb3rhq macOS packages in this section of our documentation.
...
Cyb3rhq provides an automated way of building macOS packages, keep in mind that to build an macOS package you must run this tool in an macOS system.
...
If ``Packages`` and ``Brew`` are not already installed in your system, they can be installed using the generate_cyb3rhq_packages.sh script

Download our cyb3rhq-packages repository from GitHub and go to the macos directory.

.. code-block:: console

  $ git clone https://github.com/cyb3rhq/cyb3rhq && cd cyb3rhq/packages/macos && git checkout v|CYB3RHQ_CURRENT_OSX|
...
.. code-block:: console
  ...
  # ./generate_cyb3rhq_packages.sh -s /tmp

This will build a |CYB3RHQ_CURRENT_OSX| Cyb3rhq agent macOS  package and store it in ``/tmp``.

.. code-block:: console

  # ./generate_cyb3rhq_packages.sh -s /tmp -j 6

This will also build a |CYB3RHQ_CURRENT_OSX| Cyb3rhq agent macOS package and store it in ``/tmp`` but will use 6 jobs to compile the sources.

.. code-block:: console

  # ./generate_cyb3rhq_packages.sh -s /tmp -j 6 -c
...
.. code-block:: console
  ...
  $ sudo ./generate_cyb3rhq_packages.sh -j 4 -r 1 --notarize \
...
The result of the notarization will be stored in cyb3rhq-packages/macos/request_result.txt.
