...
#. Right-click on the ``LocalDumps`` key and choose **New** > **Key**. Name the new key ``cyb3rhq-agent.exe``.

#. Right-click inside the ``cyb3rhq-agent.exe`` key and choose **New** > **Expandable String Value**. Name the new value ``DumpFolder``.

#. Right-click the ``DumpFolder`` value and select **Modify**. Change it to ``%LOCALAPPDATA%\Cyb3rhqCrashDumps``.

#. Right-click inside the ``cyb3rhq-agent.exe`` key again and choose **New** > **DWORD (32-bit) Value**. Name the new value ``DumpType``.

#. Right-click the ``DumpType`` value and select **Modify**. Change it to  ``2``.

#. Close the regedit tool and restart the Cyb3rhq agent using PowerShell with administrator privileges.
