...
.. meta::
  :description: Cyb3rhq 3.0.0 has been released. Check out our release notes to discover the changes and additions of this release.
...
This section shows the most relevant new features of Cyb3rhq v3.0.0. You will find more detailed information in our `changelog <https://github.com/cyb3rhq/cyb3rhq/blob/v3.0.0/CHANGELOG.md>`_ file.

For deploying your Cyb3rhq environment see the :doc:`Installation guide <../installation-guide/index>`.

**New features:**

- `Grouping agents`_
- `Remote agent upgrades`_
- `Cyb3rhq cluster for managers`_
...
Support for the grouping of agents has now been included at the Cyb3rhq manager level, which makes centralized configuration more flexible and efficient.
...
This groups management feature is available via terminal using a CLI included in the Cyb3rhq manager, as well using Cyb3rhq API requests.
...
send a compressed and signed WPK (Cyb3rhq Signed Package) that contains the necessary binaries and instructions to upgrade the agent.
...
WPK files will be generated by Cyb3rhq for every new release. You may also use your own custom WPK files.
...
Cyb3rhq cluster for managers
--------------------------

The Cyb3rhq cluster provides new capability to scale Cyb3rhq horizontally by adding as many manager nodes as needed to process events from the reporting agents.
...
More information on this new functionality can be found in the dedicated section at :doc:`Cluster basics </user-manual/manager/cyb3rhq-server-cluster>`.

Automatic decoding for JSON events
----------------------------------

The Cyb3rhq manager now includes a native decoder for the JSON format which can read any JSON event and extract its fields dynamically. This new decoder enables Cyb3rhq to use all JSON fields/values for creating rules.
...
The integration with VirusTotal as a threat intelligence source, along with the existing FIM capabilities is a significant improvement in Cyb3rhq's malware detection.
...
The procedure for using the MSI installer can be found at: :doc:`Install Cyb3rhq agent on Windows </installation-guide/cyb3rhq-agent/cyb3rhq-agent-package-windows>`


Cyb3rhq API
---------

The Cyb3rhq API now includes functionality to manage all the features included in this release, such as:

- the management of remote agent upgrades,
- the requests for managing groups, and
- the management of the new Cyb3rhq Cluster.

In addition, more new features can be found in the `API changelog <https://github.com/cyb3rhq/cyb3rhq-api/blob/3.0/CHANGELOG.md>`_.
...
For details on changes in the Ruleset, please visit the `Ruleset changelog <https://github.com/cyb3rhq/cyb3rhq-ruleset/blob/3.0/CHANGELOG.md>`_.
...
Additional features have been added to Cyb3rhq 3.0.0 in order to improve its performance, including, but not limited to:
