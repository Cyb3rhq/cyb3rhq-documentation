...
.. meta::
  :description: Cyb3rhq 3.7.0 has been released. Check out our release notes to discover the changes and additions of this release.
...
- `cyb3rhq/cyb3rhq <https://github.com/cyb3rhq/cyb3rhq/blob/v3.7.0/CHANGELOG.md>`_
- `cyb3rhq/cyb3rhq-api <https://github.com/cyb3rhq/cyb3rhq-api/blob/v3.7.0/CHANGELOG.md>`_
- `cyb3rhq/cyb3rhq-ruleset <https://github.com/cyb3rhq/cyb3rhq-ruleset/blob/v3.7.0/CHANGELOG.md>`_
- `cyb3rhq/cyb3rhq-kibana-app <https://github.com/cyb3rhq/cyb3rhq-kibana-app/blob/v3.7.0-6.4.3/CHANGELOG.md>`_
- `cyb3rhq/cyb3rhq-splunk <https://github.com/cyb3rhq/cyb3rhq-splunk/blob/v3.7.0-7.2.0/CHANGELOG.md>`_
...
The new ``azure-logs`` module for Cyb3rhq has the ability to obtain and read Azure logs through several service APIs. This helps to monitor all the activity happening in the infrastructure, just by setting up the module to monitor the virtual machines that form the infrastructure, sending events to the Cyb3rhq manager for analysis.

There are several ways to monitor the Azure instances:

  - **Installing the Cyb3rhq agent on the instances.**
  ...
  - **Monitoring the Azure Active Directory service.** Monitoring management actions such as creation, update or deletion of users. It's possible to receive alerts on the Cyb3rhq manager when some of these events occur on the Azure infrastructure.
...
The new ``docker`` module for Cyb3rhq makes easier to monitor and collect the activity from Docker containers such as creation, running, starting, stopping or pausing events.

In addition to this, and as always, the Cyb3rhq agent can be used to monitor more services and events from the Docker servers, like **File integrity** or **Log data collection**.

In this example, the Docker command ``docker pause apache`` will stop the container ``apache`` and will trigger an alert, as seen on the screenshot below from the Cyb3rhq app for Kibana:
...
.. thumbnail:: ../images/release-notes/3.7.0/kibana-remote-query.png
    :title: Query agent configuration using the Cyb3rhq app
    :align: center
    :width: 100%
...
In this version, the Cyb3rhq API includes a new filtering system. The ``q`` parameter allows requesting information using advanced queries with logical operators and separators. Find a more detailed explanation of this feature in the :ref:`API queries <queries>` section.

New features for Kibana plugin
------------------------------

The Cyb3rhq app for Kibana includes new features and interface redesigns to make use of the new features included in this version:
  ...
  - The new :ref:`Osquery <osquery>` extension shows scans results from this Cyb3rhq module.
...
Cyb3rhq 3.7.0 introduces an update to the Elasticsearch template. This will cause a **breaking change** in existing installations, although new installations **won't be affected** by this error.
