...
.. meta::
   :description: The Cyb3rhq agent is multi-platform and runs on monitored systems providing threat prevention, detection, and response capabilities.

Cyb3rhq agent
===========

The Cyb3rhq agent runs on Linux, Windows, macOS, Solaris, AIX, and other operating systems. It can be deployed to laptops, desktops, servers, cloud instances, containers, or virtual machines. The agent helps to protect your system by providing threat prevention, detection, and response capabilities. It is also used to collect different types of system and application data that it forwards to the :doc:`Cyb3rhq server <cyb3rhq-server>` through an encrypted and authenticated channel.

Agent architecture
------------------

The Cyb3rhq agent has a modular architecture. Each component is in charge of its own tasks, including monitoring the file system, reading log messages, collecting inventory data, scanning the system configuration, and looking for malware. Users can manage agent modules via configuration settings, adapting the solution to their particular use cases.
...
-  **Command execution:** Agents run authorized commands periodically, collecting their output and reporting it back to the Cyb3rhq server for further analysis. You can use this module for different purposes, such as monitoring hard disk space left or getting a list of the last logged-in users.
...
Communication with Cyb3rhq server
-------------------------------

The Cyb3rhq agent communicates with the :doc:`Cyb3rhq server <cyb3rhq-server>` to ship collected data and security-related events. Besides, the agent sends operational data, reporting its configuration and status. Once connected, the agent can be upgraded, monitored, and configured remotely from the Cyb3rhq server.
