...
.. meta::
   :description: In this integration guide, you can find how to integrate your Cyb3rhq deployment with Elastic Stack, OpenSearch, and Splunk.

Integrations guide: Elastic, OpenSearch, Splunk, Amazon Security Lake
=====================================================================

Cyb3rhq offers extensive compatibility and robust integration features that allow users to connect it with other security solutions and platforms. Integrating Cyb3rhq with other security solutions enables users to manage Cyb3rhq data in diverse ways.
...
.. _cyb3rhq_indexer_integration:

Cyb3rhq indexer integration
^^^^^^^^^^^^^^^^^^^^^^^^^

.. thumbnail:: /images/integrations/indexer-integration-diagram.png
   :title: Cyb3rhq indexer integration diagram
   :align: center
   :width: 80%

The Cyb3rhq agent collects security data from monitored endpoints. These data are analyzed by the Cyb3rhq server and indexed in the Cyb3rhq indexer. The Cyb3rhq indexer integration forwards analyzed security data from the Cyb3rhq indexer to the third-party security platform indexer in the form of indexes. Indexes are collections of documents with similar properties.

Cyb3rhq indexer integration requires a running Cyb3rhq indexer and uses `Logstash <https://www.elastic.co/logstash/>`__ as a :ref:`data forwarder <data_forwarders>` for forwarding alerts to a third-party security platform. Logstash forwards the alerts generated after querying the Cyb3rhq indexer. This integration requires that you install Logstash on a dedicated server or on the server hosting the third-party indexer.

We recommend the Cyb3rhq indexer integration if you want to continue analyzing security data in Cyb3rhq and archive it in a data lake for storage or out-of-band analysis. The Cyb3rhq indexer and third-party platform indexer both create indexes for the same alerts. This integration results in the redundancy of indexes which can lead to high resource costs.

.. _cyb3rhq_server_integration:

Cyb3rhq server integration
^^^^^^^^^^^^^^^^^^^^^^^^

.. thumbnail:: /images/integrations/server-integration-diagram.png
   :title: Cyb3rhq server integration diagram
   :align: center
   :width: 80%

The Cyb3rhq agent collects security data from monitored endpoints. The Cyb3rhq server analyzes the data and generates alerts on the Cyb3rhq dashboard. Cyb3rhq stores security alerts locally in the ``/var/ossec/logs/alerts/alerts.log`` and ``/var/ossec/logs/alerts/alerts.json`` alerts files. The Cyb3rhq server integration operates by reading the ``/var/ossec/logs/alerts/alerts.json`` file and forwarding the alerts to the third-party platform using a :ref:`data forwarder <data_forwarders>`.

We recommend Cyb3rhq server integration over indexer integration if you don’t have enough resources for hosting both the third-party security platform indexer and the Cyb3rhq indexer. This is particularly relevant for users operating Cyb3rhq at a large scale, generating numerous alerts.

To implement the Cyb3rhq server integration, a data forwarder must be installed on the same system as the Cyb3rhq server. In multi-node configurations, the data forwarder should be installed on each Cyb3rhq server node.

.. note::

   To make sure Cyb3rhq logs the alerts to ``/var/ossec/logs/alerts/alerts.json``, check that the :ref:`global_jsonout_output` option in the Cyb3rhq server configuration ``/var/ossec/etc/ossec.conf`` file is set to ``yes``.
...
Third-party security platforms typically have a dedicated data collector or data forwarder which is required for implementing these integrations. These forwarders facilitate the data flow from the Cyb3rhq indexer or server to the third-party security platforms.

In the Cyb3rhq indexer integration alternative, the forwarder executes periodic queries to the Cyb3rhq indexer. These queries are performed in blocks, with the time range of each query matching the specified query period. Consequently, there may be a delay in the arrival of alerts at the destination, depending on the frequency set for querying new alerts.

.. thumbnail:: /images/integrations/indexer-queries-diagram.png
   :title: Cyb3rhq indexer queries diagram
   :align: center
   :width: 80%

In the Cyb3rhq server integration, the forwarder periodically checks the Cyb3rhq alerts file. If it detects changes since the last read, it loads all the new alerts and sends them to the third-party platform.
...
Logstash supports reading data from indexes using  an input plugin and supports transformations with an output plugin. In summary, Logstash is the most compatible data forwarder for the Cyb3rhq indexer and server integrations.
...
When integrating Cyb3rhq with other security solutions, you need to carefully plan your storage resources and escalation needs beforehand. While the details on capacity planning are out of the scope of this guide, here are a few considerations.

When using the Cyb3rhq indexer integration, you need to put the following points into consideration:
...
-  The scalability of the data forwarder infrastructure to read all the data from the Cyb3rhq indexer and forward it to the third-party security platform.

When using the Cyb3rhq server integration, you need to put the following points into consideration:

-  The disk space each Cyb3rhq server must have to forward the data.
-  The network bandwidth the Cyb3rhq server and the third-party security platform need to receive and forward the security data respectively. 
...
After the integration, you might have the same data in both Cyb3rhq and third-party security platforms. To minimize the amount of duplicated data, you can archive the data from the Cyb3rhq server or indexer for storage or later analysis. Also, you need to plan backups and recovery of data in case of failure. 
