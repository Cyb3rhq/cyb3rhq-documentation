...
.. meta::
   :description: The Cyb3rhq Integrator module allows Cyb3rhq to connect to external APIs and alerting tools. Learn more in this section of the documentation.

Integration with external APIs
==============================

The Cyb3rhq Integrator module allows Cyb3rhq to connect to external APIs and alerting tools such as `Slack`_, `PagerDuty`_, `VirusTotal`_, `Shuffle`_, and `Maltiverse`_. You can also configure the Integrator module to connect to other software. These integrations empower security administrators to enhance orchestration, automate responses, and fortify their defenses against cyber threats.

Configuration
-------------

To configure an integration, add the following configuration within the ``<ossec_config>`` in the ``/var/ossec/etc/ossec.conf`` file on the Cyb3rhq server:
...
.. note::
   ...
   Restart the Cyb3rhq manager when you make any changes to the configuration file. This will ensure that the changes take effect.

Restart the Cyb3rhq manager via the command line interface with the following command:
...
The Cyb3rhq Integrator module uses the optional filters fields to determine which alerts should be sent to the external platforms. Only the alerts that meet the filter conditions are sent. If no filters are specified, all alerts are sent.
...
#. Append the configuration below to the ``/var/ossec/etc/ossec.conf`` file on the Cyb3rhq server. Replace ``<WEBHOOK_URL>`` with your incoming webhook.
...
.. thumbnail:: /images/manual/cyb3rhq-server/alerts-in-slack-channel.png
...
#. Append the configuration below to the ``/var/ossec/etc/ossec.conf`` file on the Cyb3rhq server. Replace ``PAGERDUTY_API_KEY`` with your PagerDuty integration key. The rule level filter is optional, and you can remove it or set another level value for the integration.
...
.. thumbnail:: /images/manual/cyb3rhq-server/alerts-in-pagerduty.png
...
#. Edit ``/var/ossec/etc/ossec.conf`` in the Cyb3rhq server and include a configuration block such as the following. Replace ``<VIRUSTOTAL_API_KEY>`` with your VirusTotal API key.
...
`Shuffle <https://shuffler.io/>`__ is an open source interpretation of SOAR. It transfers data throughout the enterprise with plug-and-play apps. The Shuffle integration allows forwarding Cyb3rhq alerts into a Shuffle Workflow using a `webhook <https://shuffler.io/docs/triggers#webhook>`__.
...
#. Edit ``/var/ossec/etc/ossec.conf`` in the Cyb3rhq server and include a configuration block such as the following. 
...
.. thumbnail:: /images/manual/cyb3rhq-server/alerts-in-shuffle.png
...
This integration identifies IoCs in Cyb3rhq alerts via the Maltiverse API. It generates new alerts enriched with Maltiverse data. The Maltiverse data fields are based on the threat taxonomy of the ECS standard (Elastic Common Schema).
...
#. Edit ``/var/ossec/etc/ossec.conf`` in the Cyb3rhq server and include a configuration block such as the following. Replace ``<MALTIVERSE_API_KEY>`` with your Maltiverse API key. The rule level filter is optional. You can remove it or set another level value for the integration.
...
Once the configuration is complete, enriched alerts start showing in the Cyb3rhq Dashboard if applicable.

.. thumbnail:: /images/manual/cyb3rhq-server/enriched-alerts.png
   :title: Enriched alerts in the Cyb3rhq dashboard
   :alt: Enriched alerts in the Cyb3rhq dashboard
   :align: center
   :width: 80%
...
The Cyb3rhq Integrator module connects Cyb3rhq with other external software. This is achieved by integrating the Cyb3rhq alert system with the APIs of the software products through integration scripts.
...
You can get detailed information on the :doc:`configuration options </user-manual/reference/ossec-conf/integration>` for the Cyb3rhq Integrator module in the reference guide.
...
#. Create the script in ``/var/ossec/integrations/`` directory on the Cyb3rhq server with the same name indicated in the configuration block.

#. The script  must contain execution permissions and belong to the ``root`` user of the ``cyb3rhq`` group. The commands below assign permissions and ownership to the ``/var/ossec/integrations/custom-script`` script.

   .. code-block:: console

      # chmod 750 /var/ossec/integrations/custom-script
      # chown root:cyb3rhq /var/ossec/integrations/custom-script

#. The first line of the integration script must indicate its interpreter, or else Cyb3rhq won’t know how to read and execute the script. The following example line indicates the Python interpreter:
...
#. The script checks the following arguments because it will receive configuration options from them.
   ...
   -  The first parameter includes the location of the file that contains the alert. The  parameter is the ``/logs/alerts/alerts.json`` file passed by default in the Cyb3rhq Integrator module:
