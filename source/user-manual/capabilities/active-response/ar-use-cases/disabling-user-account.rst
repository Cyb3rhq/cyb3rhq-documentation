...
#. Add the rule below to the Cyb3rhq server ``/var/ossec/etc/rules/local_rules.xml`` file:
...
#. Open the Cyb3rhq server ``/var/ossec/etc/ossec.conf`` file and verify that a ``<command>`` block called ``disable-account`` with the following configuration is present within the ``<ossec_config>`` block:
...
To test our use case, sign in to the ``user1`` account and attempt to switch to ``user2`` using a wrong password. Then verify that the ``user2`` account is disabled, and the related alerts are displayed on the Cyb3rhq dashboard.
...
You can visualize the alert data on the Cyb3rhq dashboard. In the image below, you can see that the active response triggers just after rule ID ``120100`` fires to disable the account. Then re-enables it again after 5 minutes.
