...
.. meta::
  :description: You can configure the FIM capability on the Cyb3rhq server and the Cyb3rhq agent. Learn more about it in this section of the Cyb3rhq documentation. 
  
Basic settings
==============

You can configure the FIM capability on the Cyb3rhq server and the Cyb3rhq agent. A :ref:`default FIM configuration <reference_ossec_syscheck_default_configuration>` exists on both the Cyb3rhq server and the Cyb3rhq agent. You can modify these settings depending on your needs. 

You can configure the FIM module on the Cyb3rhq server and the Cyb3rhq agent :doc:`configuration </user-manual/reference/ossec-conf/index>`  file. You can also configure this capability remotely using the :doc:`centralized configuration </user-manual/reference/centralized-configuration>` file. The list of all FIM configuration options is available in the syscheck section.

In this guide, we show different configuration options that the Cyb3rhq FIM module supports.
...
.. note::
   ...
   When specifying a directory for real time monitoring, it must exist before restarting the Cyb3rhq agent. If not, the module ignores the directory until it finds it on a subsequent restart of the Cyb3rhq agent.
...
You must use the ``report_changes`` attribute with caution when you enable this option.  Cyb3rhq copies every monitored file to a private location increasing storage usage. You can find the copy of the files at: 
...
#. Restart the Cyb3rhq agent with administrator privilege to apply the configuration changes:
...
An alternative method is using rules of alert level 0. This method ignores the alerting of specific files and directories scanned by the FIM module. Alerts for level 0 rules are silent and the Cyb3rhq server doesn’t report them. 
...
#. Add the following settings to the Cyb3rhq agent ``/var/ossec/etc/ossec.conf`` configuration file:
...
#. Create the ``fim_ignore.xml`` file in the ``/var/ossec/etc/rules/`` directory on the Cyb3rhq server:
...
3. Restart the Cyb3rhq manager to apply the configuration changes:
