...
#. Enable remote command execution on the Windows endpoint by appending the following settings to the ``C:\Program Files (x86)\ossec-agent\local_internal_options.conf`` file:
   ...
   .. warning:: Enable remote command execution with caution as this gives the Cyb3rhq user permission to run any command on the endpoint.
...
#. Append the following configuration to the ``/var/ossec/etc/shared/default/agent.conf`` file on the Cyb3rhq server:
...
Go to **Threat Hunting** module on the Cyb3rhq dashboard to visualize the generated alert when the free disk space is less than 20%.
...
#. Create a bash script named ``disk-usage.sh`` in the ``/var/ossec/etc/shared/default/`` directory of the Cyb3rhq server and add the following content. Cyb3rhq pushes the script to the ``/var/ossec/etc/shared/`` directory on the Linux endpoint where it is executed. The script adds a ``disk-usage`` header to the output of the ``df -h`` command:
...
#. Append the following configuration to the ``/var/ossec/etc/shared/default/agent.conf`` file on the Cyb3rhq server. Replace ``<MD5_HASH>``, ``<SHA1_HASH>``, and ``<SHA256_HASH>`` with the appropriate hashes of the ``/bin/bash`` shell binary on the Linux endpoint:
...
#. Add the following decoder to the ``/var/ossec/etc/decoders/local_decoder.xml`` file on the Cyb3rhq server:
...
#. Add the following rules to the ``/var/ossec/etc/rules/local_rules.xml`` file on the Cyb3rhq server. The rule generates an alert when the disk usage of the ``/dev`` partition exceeds ``80%``:
...
Go to **Threat Hunting** module on the Cyb3rhq dashboard to visualize the generated alerts when the disk usage of the ``/dev`` partition exceeds ``80%``.
...
For this endpoint, we monitor the disk space using the ``df -P`` command. Cyb3rhq triggers a rule to generate an alert when the disk usage of the ``/dev`` partition is ``100%``. We use the Logcollector module to demonstrate this use case. You can also use the Command module.
...
Cyb3rhq has an out-of-the-box rule with ID ``531`` that generates an alert when the disk usage of the ``/dev`` partition is ``100%``. The rule is defined below and is found in the `Cyb3rhq GitHub repository <https://github.com/cyb3rhq/cyb3rhq-ruleset/blob/master/rules/0015-ossec_rules.xml>`__.
...
.. note::
   ...
   - After viewing the generated alert on the Cyb3rhq dashboard, you can remove the file ``big_file`` to regain your disk space.

Visualize the alerts
^^^^^^^^^^^^^^^^^^^^

Go to **Threat Hunting** module on the Cyb3rhq dashboard to visualize the generated alert when the disk usage of the ``/dev`` partition is ``100%``.
