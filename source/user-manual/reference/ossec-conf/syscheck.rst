...
.. meta::
  :description: The ossec.conf file is the main configuration file on the Cyb3rhq manager and also important on the agents. Learn more about FIM settings with configuration examples here.
...
This option can be used to limit the size of the ``queue/diff/local`` folder where Cyb3rhq stores the compressed files used to perform the diff operation when ``report_changes`` is enabled. After reaching this size, alerts will not show the diff information until the size is smaller than the configured limit.
...
Sets up the FIM engine to collect the Audit events using keys with ``audit_key``. Cyb3rhq will include in its FIM baseline those events being monitored by Audit using `audit_key`. For those systems where Audit is already set to monitor folders for other purposes, Cyb3rhq can collect events generated as a key from `audit_key`. This option is only available for **Linux systems with Audit**.
...
.. tabs::
 ...
 .. group-tab:: Cyb3rhq agent - Linux/Unix
 ...
 .. group-tab:: Cyb3rhq agent - Windows
 ...
 .. group-tab:: Cyb3rhq agent - MacOS X
