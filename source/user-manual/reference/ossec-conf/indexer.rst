...
Configure these options to forward inventory data to the Cyb3rhq indexer.
...
Enables forwarding inventory data to the Cyb3rhq indexer.
...
Cyb3rhq indexer nodes to connect to. Use the ``host`` option for setting up each node connection.

host
~~~~

Cyb3rhq indexer node URL or IP address to connect to. For example, ``http://172.16.1.11`` or ``192.168.3.2:9230``.
