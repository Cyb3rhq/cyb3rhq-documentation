...
.. meta::
  :description: Learn more about how to configure the collection of log data from files, Windows events, and from the output of commands with Cyb3rhq.
...
Wildcards can be used on Linux and Windows systems, if the log file doesn't exist at ``cyb3rhq-logcollector`` start time, such log will be re-scanned after ``logcollector.vcheck_files`` seconds.
...
.. code-block:: xml
  ...
  <localfile>
      <location>C:\Users\cyb3rhq\myapp\*</location>
...
It allows to read new log content since ``cyb3rhq-logcollector`` was stopped.

By default, when ``cyb3rhq-logcollector`` is started it reads the logs generated since that moment.
Set it to ``no`` to collect events generated since ``cyb3rhq-logcollector`` was stopped.
...
.. note::
  If the log rotates while ``cyb3rhq-logcollector`` is stopped and ``only-future-events`` is set to ``no``, it will start reading from the beginning of the log.
...
This label can be used to filter *Windows* ``eventchannel`` events or *macOS* ULS logs (``macos``) that Cyb3rhq will process.
...
Sample Log message as analyzed by cyb3rhq-analysisd:
...
In the following configuration example Cyb3rhq collects the ``journald`` logs if any of the following conditions are met.
