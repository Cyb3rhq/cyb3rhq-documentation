...
.. note::
  Since Cyb3rhq v3.10.0, although this section is missing, the Cyb3rhq Agent will run scans for every policy (.yaml or .yml files) present in their ruleset folder.

.. warning::
  Since Cyb3rhq v4.2.0, when a policy is defined by a relative path, this path is relative to the Cyb3rhq installation directory. If the policy is located outside the installation directory, a full path can be used.
