...
.. meta::
   :description: This Cyb3rhq documentation section provides instructions for monitoring the Office 365 audit log for your organization
...
This section provides instructions for monitoring the Office 365 audit log for your organization. The audit log provides information on the changes and user activities taking place in an Office 365 environment. Cyb3rhq allows you to monitor the following activities in Office 365:
...
The Office 365 Management APIs provide a platform for various management tasks, including service communications, security, compliance, reporting, and auditing. It offers an interface for collecting audit logs from an Office 365 environment. Cyb3rhq collects audit logs from Office 365 using this interface.
...
Cyb3rhq needs to authenticate to the Office 365 Management API to connect and pull audit logs for analysis. This process is achieved by registering an application on the Microsoft Azure portal to obtain the required credentials.

You need the following requirements to access the audit logs of Office 365 with Cyb3rhq:
...
The Office 365 API provides an endpoint for accessing audit logs in Office 365. You need an application with the right permissions to access the Microsoft API. The following list provides a summary of the steps you need to perform on Microsoft Azure to integrate with Cyb3rhq:
...
Setting up Cyb3rhq for Office 365 monitoring
------------------------------------------

This section delves into the processes involved in configuring Cyb3rhq for effective monitoring of Office 365 environments. The various aspects of the configuration process include the integration with Office 365 APIs for log collection, the activation of the dashboard visualization module for Office 365 events, and the correlation of rules.

Configuring Cyb3rhq with Office 365 APIs
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

The Cyb3rhq module for Office 365 pulls audit logs from the Office 365 APIs for analysis and rule correlation. You can configure the module on either the Cyb3rhq server or the Cyb3rhq agent. It is recommended to configure it on the Cyb3rhq agent to reduce the workload on the Cyb3rhq server, thereby improving the performance of your monitoring infrastructure.

Perform the following steps to configure the Cyb3rhq server to pull audit logs from an Office 365 environment.
...
#. Append the following configuration to the ``/var/ossec/etc/ossec.conf`` file. The configuration pulls only the ``Audit.SharePoint`` type of events within an interval of ``1m``.
   ...
   -  ``<only_future_events>`` specifies the Cyb3rhq module for Office 365 to collect only events generated after you start the Cyb3rhq manager when the value is set to ``yes``. When the value is set to no, it collects previous events generated before you start the Cyb3rhq manager. The default value is ``yes``, and the allowed values are ``yes`` and ``no``.
   ...
   -  The ``<subscriptions>`` block configures the internal options in the Office 365 REST API.
      ...
      -  ``<subscription>`` specifies the content types from which Cyb3rhq collects audit logs. The :ref:`subscription types <office365_module_subscriptions_subscription>` that can be configured include ``Audit.AzureActiveDirectory``, ``Audit.Exchange``, ``Audit.SharePoint``, ``Audit.General``, and ``DLP.All``.

   To learn more about the configuration options, kindly check the :doc:`Cyb3rhq module for Office 365 </user-manual/reference/ossec-conf/office365-module>` reference guide.
...
You can configure Cyb3rhq to monitor multiple tenants in an organization by specifying the organization’s credentials (``<tenant_id>``, ``<client_id>``, ``<client_secret>``, and ``<api_type>``) in individual ``<api_auth>`` blocks.
...
Cyb3rhq pulls audit logs from the following subscription types in Office 365:
...
You can configure Cyb3rhq to monitor multiple subscriptions in an organization tenant(s) by specifying the subscription type in individual ``<subscription>`` tags within the same ``<subscriptions>`` block.
...
The Cyb3rhq dashboard has an Office 365 module that provides detailed information and insights about the events that occur in Office 365. The module provides three visualization options.

-  **Dashboard**
-  **Panel**
-  **Events**

To select any of them, navigate to the  **Office 365** tab on the **Cloud security** section of the Cyb3rhq dashboard.
...
When a user logs into the Microsoft Azure AD, the action generates an event. You can configure Cyb3rhq to monitor and visualize these events by performing the following actions:

#. Append the following configuration to the ``/var/ossec/etc/ossec.conf`` file on the Cyb3rhq server:
...
#. Visit the Cyb3rhq dashboard and navigate to **Office 365**, then click on the **Events** tab to view the generated alerts.
   ...
   .. code-block:: none
      ...
      {
        "_index": "cyb3rhq-alerts-4.x-2024.01.29",
        ...
        "highlight": {
          "manager.name": [
            "@opensearch-dashboards-highlighted-field@cyb3rhq-server@/opensearch-dashboards-highlighted-field@"
          ],
          "rule.groups": [
...
Perform the following steps to configure the Cyb3rhq server for monitoring admin activities on Microsoft Azure AD.
...
We create and delete a user account on the Microsoft Azure AD to generate an activity for Cyb3rhq to monitor and display on the dashboard.
