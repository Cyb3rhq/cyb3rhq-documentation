...
.. meta::
  :description: Learn about some considerations that must be taken into account when configuring the Cyb3rhq module for AWS.
...
If the S3 bucket contains a long history of logs and its directory structure is organized by dates, it's possible to filter which logs will be read by Cyb3rhq. There are multiple configuration options to do so:
...
* ``aws_account_id``: **This option will only work on CloudTrail, VPC, and Config buckets**. If you have logs from multiple accounts, you can filter which ones will be read by Cyb3rhq. You can specify multiple ids separating them by commas.
* ``regions``: **This option will only work on CloudTrail, VPC and Config buckets, and Inspector service**. If you have logs from multiple regions, you can filter which ones will be read by Cyb3rhq. You can specify multiple regions separating them by commas. It is mandatory to specify the region when configuring an S3 bucket from an AWS GovCloud region (available GovCloud regions are ``us-gov-east-1`` and ``us-gov-west-1``).
...
The ``aws-s3`` Cyb3rhq module only looks for new logs in buckets based on the key of the last processed log object, which includes the datetime stamp. If older logs are loaded into the S3 bucket or the :ref:`only_logs_after <only_logs_aws_buckets>` option date is set to a datetime earlier than previous executions of the module, the older log files will be ignored and not ingested into Cyb3rhq.
...
Find an example of running the module on a manager using the ``--reparse`` option. ``/var/ossec`` is the Cyb3rhq installation path.

.. code-block:: console

  # /var/ossec/wodles/aws/aws-s3 -b 'cyb3rhq-example-bucket' --reparse --only_logs_after '2021-Jun-10' --debug 2
...
Cyb3rhq supports additional configuration options found in the ``.aws/config file``. The supported keys are the primary keys stated in the `boto3 configuration <https://boto3.amazonaws.com/v1/documentation/api/latest/guide/configuration.html>`_. Supported keys are:
...
To configure multiple profiles for the integration, declare each profile section in ``~/.aws/config`` with ``[profile <PROFILE-NAME>]``. If you don't declare a profile section in this configuration file, Cyb3rhq uses the ``default`` profile.
...
.. code-block:: xml
  ...
  <wodle name="aws-s3">
    ...
    <bucket type="cloudtrail">
      <name>cyb3rhq-cloudtrail-2</name>
      <aws_profile>default</aws_profile>
      <iam_role_arn>arn:aws:iam::xxxxxxxxxxx:role/cyb3rhq-role</iam_role_arn>
