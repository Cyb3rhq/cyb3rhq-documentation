...
.. meta::
  :description: AWS CloudTrail is a service supported by Cyb3rhq that enables auditing AWS accounts. Check out this section of our documentation to learn more about it. 
...
#. Open the Cyb3rhq configuration file (``/var/ossec/etc/ossec.conf``) and add the following configuration block to enable the integration with CloudTrail:
...
#. Restart Cyb3rhq in order to apply the changes:

    * If you're configuring a Cyb3rhq manager:

      .. include:: /_templates/common/restart_manager.rst

    * If you're configuring a Cyb3rhq agent:
...
Below are some use cases for Cyb3rhq rules built for EC2.

Run a new instance in EC2
+++++++++++++++++++++++++

When a user runs a new instance in EC2 an AWS event is generated. As previously mentioned, the log message is collected by the Cyb3rhq agent, and forwarded to the manager for analysis. The following alert will be shown in the Cyb3rhq dashboard, it shows data such as instance type, the user who created it, or creation date:
...
When an instance in EC2 is started, the following alert will be shown on the Cyb3rhq dashboard, it shows information such as the instance id and the user who started it:
...
When a new security group is created, the following alert is shown on the Cyb3rhq dashboard. It shows information such as the user who created it and information about the security group:
...
Below are some use cases for Cyb3rhq alerts built and used for IAM events.

Create a user account
+++++++++++++++++++++

When we create a new user account in IAM an AWS event is generated. As previously mentioned, the log message is collected by the Cyb3rhq agent, and forwarded to the manager for analysis. When a user account is created, the following alert will appear on the Cyb3rhq dashboard. You can see the username of the created user and who created it:
...
If an unauthorized user attempts to create new users, the following alert will be shown in the Cyb3rhq dashboard. It will show you which user has tried to create a user account and the username it tried to create:
...
When a user tries to log in with an invalid password, the following alert will be shown in the Cyb3rhq dashboard. There will be shown data such as the user who tried to log in or the browser it was using:
...
When more than 4 authentication failures occur in a **360** second time window, Cyb3rhq raises this alert:
...
After a successful login, the following event will be shown in the Cyb3rhq dashboard. It shows the user who logged in, the browser it used, and other useful information:

.. thumbnail:: /images/cloud-security/aws/aws-login-5.png
  :align: center
  :width: 70%

Here are the Cyb3rhq dashboard charts for IAM events:
