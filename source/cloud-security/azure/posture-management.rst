...
.. meta::
   :description: Use Cyb3rhq to monitor Microsoft Azure security posture.
...
Cyb3rhq is a free, open source, enterprise-grade security monitoring platform that provides comprehensive protection for cloud, on-premises, containerized, and virtualized environments. Microsoft Azure is a comprehensive cloud computing platform that offers a wide range of services to help businesses build, deploy, and manage their applications and infrastructure.

This section demonstrates how to use Cyb3rhq to monitor Microsoft Azure security posture.

Integrating Cyb3rhq with Microsoft Azure
--------------------------------------

Cyb3rhq integrates with Azure using the Log Analytics Workspace. The Azure Log Analytics workspace is a unique environment for storing log data from Azure Monitor and other Azure services, such as the Microsoft Defender for Cloud. Cyb3rhq provides a native :doc:`integration module for Azure </cloud-security/azure/activity-services/services/log-analytics>` that retrieves logs from the Log Analytics Workspace.

Below is a summary of the actions performed on Azure to integrate with Cyb3rhq.
...
-  **Creating a Log Analytics workspace**: The workspace is where logs and data are stored, and it has a unique workspace ID and resource ID. The Cyb3rhq :doc:`Azure module </user-manual/reference/ossec-conf/wodle-azure-logs>` is then configured to query the workspace for new data. 
...
Create a Microsoft Entra ID application for Cyb3rhq authentication to the Log Analytics Workspace. Microsoft Entra ID is the identity directory service from Microsoft.
...
Create a Log Analytics Workspace that enables Cyb3rhq to retrieve log data from Azure. 
...
#. In the search bar of the Azure portal, type ``Log Analytics workspaces``, select the new workspace. Copy the **Workspace ID** from the **Essentials** section. The Workspace ID will be used as part of the configuration in Cyb3rhq.
...
#. Copy the Azure tenant **Primary domain** _`name` from the **Basic Information** section. This will be used as part of the configuration in Cyb3rhq.
...
Configure the Cyb3rhq server to receive logs from Microsoft Azure by performing the following steps.
...
#. Append the following content to the ``/var/ossec/etc/ossec.conf`` configuration file. The configuration specifies how Cyb3rhq connects to Azure:
...
#. Create a rule file ``azure_posture.xml`` in the ``/var/ossec/etc/rules/`` directory and add the following custom rules to detect Azure posture findings:
   ...
   -  Rule ID ``100200`` is triggered when Cyb3rhq detects a new security posture recommendation in Azure.
   -  Rule ID ``100201`` is triggered when Cyb3rhq detects an attack against Azure Virtual Machine.
   -  Rule ID ``100202`` is triggered when Cyb3rhq detects an attack in Azure Key Vault.
   -  Rule ID ``100203`` is triggered when Cyb3rhq detects an attack in Azure App Service.
   -  Rule ID ``100204`` is triggered when Cyb3rhq detects an attack in Azure API Management.
   -  Rule ID ``100205`` is triggered when Cyb3rhq detects an attack in Azure Container and clusters.
...
Posture management results on the Cyb3rhq dashboard
-------------------------------------------------

Visualize the results by navigating to the **Modules** > **Security events** tab. Filter for the azure rule group.

.. thumbnail:: /images/cloud-security/azure/azure-security-alerts-on-cyb3rhq-dashboard.png
   :title: Azure security alerts on the Cyb3rhq dashboard
   :alt: Azure security alerts on the Cyb3rhq dashboard
   :align: center
   :width: 80%
