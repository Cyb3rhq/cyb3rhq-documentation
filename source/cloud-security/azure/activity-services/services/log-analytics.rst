...
.. meta::
  :description: Azure Log Analytics is a service that monitors Azure infrastructures offering query capabilities. Learn how to use Log Analytics with Cyb3rhq in this section.
...
#. In the **Overview** section, copy the ``Workspace Id`` value. The Cyb3rhq configuration needs it to make requests to the API.
...
Cyb3rhq requires valid credentials to pull logs from Log Analytics. Take a look at the :ref:`credentials <graph_and_log_analytics_credentials>` section to learn how to generate a client secret so you can access the App registration.
...
Through the following configuration, Cyb3rhq is ready to search for any query accepted by Azure Log Analytics. This example configuration includes a representative ``tag`` and will be scheduled for every Monday at 02:00, using an offset of one day, which means only the log data from the last day will be parsed:
...
The following rules are already included in Cyb3rhq by default. With them, it it possible to monitor the infrastructure activity and get the related alerts.
...
Once the Cyb3rhq configuration is set and the ``azure-logs`` module is running using the previous configuration, the event will be processed. The results can be checked in the Cyb3rhq dashboard:
