...
.. meta::
   :description: Learn how to integrate Cyb3rhq with YARA to detect the addition of malicious files, and use ChatGPT queries to enrich the YARA scan results with additional insights.
...
In this use case, we integrate Cyb3rhq with YARA to detect when a malicious file is added to a monitored endpoint. The integration utilizes the Cyb3rhq :doc:`FIM </user-manual/capabilities/file-integrity/index>` module to monitor a directory for new or modified files. When a file modification or addition is detected, the Cyb3rhq :doc:`active response </user-manual/capabilities/active-response/index>` module triggers a YARA scan on the file. The Active response module automatically deletes the malicious file from the endpoint if it has a positive match with a malicious signature. The active response module then queries ChatGPT to enrich the YARA scan result with additional insight into the malicious file that helps security teams understand its nature, potential impact, and remediation.
...
============== ====================================================================================================================================
Ubuntu 22.04   Monitored endpoint configured with Cyb3rhq File Integrity Monitoring (FIM) module and YARA integration with ChatGPT log enrichment.
Windows 11     Monitored endpoint configured with Cyb3rhq File Integrity Monitoring (FIM) module and YARA integration with ChatGPT log enrichment.
============== ====================================================================================================================================
...
#. Change the owner of the ``yara_rules.yar`` file to ``root:cyb3rhq``, and the file permissions to ``750``:

   .. code-block:: console

      $ sudo chown root:cyb3rhq /var/ossec/active-response/yara/rules/yara_rules.yar
...
#. Create a script ``yara.sh`` in the ``/var/ossec/active-response/bin/`` directory. This script runs YARA scans on files added or modified in the monitored directories. It also queries ChatGPT to enrich the logs and attempts to remove malware files detected by YARA. 
   ...
   .. code-block:: bash
      ...
      then
          echo "cyb3rhq-YARA: ERROR - YARA active response error. YARA path and rules parameters are mandatory." >> ${LOG_FILE}
          exit 1
      fi
      ...
      then
          ...
          if rm -rf "$FILENAME"; then
              echo "cyb3rhq-YARA: INFO - Successfully deleted $FILENAME" >> ${LOG_FILE}
          else
              echo "cyb3rhq-YARA: INFO - Unable to delete $FILENAME" >> ${LOG_FILE}
          fi
          ...
          while read -r line; do
              ...
              if [[ $description != "" ]]; then
                  ...
                  if echo "$chatgpt_response" | grep -q "invalid_request_error"; then
                      ...
                      echo "cyb3rhq-YARA: ERROR - Invalid ChatGPT API key" >> ${LOG_FILE}
                      # Log Yara scan result without ChatGPT response
                      echo "cyb3rhq-YARA: INFO - Scan result: $line | chatgpt_response: none" >> ${LOG_FILE}
                  else
                      ...
                      if [[ $response_text == "null" ]]; then
                          echo "cyb3rhq-YARA: ERROR - ChatGPT API returned null response: $chatgpt_response" >> ${LOG_FILE}
                      else
                          # Combine the YARA scan output and ChatGPT response
                          combined_output="cyb3rhq-YARA: INFO - Scan result: $line | chatgpt_response: $response_text"
                  ...
                  echo "cyb3rhq-YARA: INFO - Scan result: $line" >> ${LOG_FILE}
              fi
          done <<< "$YARA_output"

          # If API key was invalid, log a specific message
          if $api_key_invalid; then
              echo "cyb3rhq-YARA: INFO - API key is invalid. ChatGPT response omitted." >> ${LOG_FILE}
          fi
      else
          echo "cyb3rhq-YARA: INFO - No YARA rule matched." >> ${LOG_FILE}
      fi
   ...
   .. note::
      ...
      If the supplied ``<API_KEY>`` is invalid, Cyb3rhq triggers an alert with the value of the ``chatgpt_response`` field set to ``None``. Logs about the invalid API key are in the ``/var/ossec/logs/active-responses.log`` file.

#. Change the owner of the ``yara.sh`` script to ``root:cyb3rhq``, and the file permissions to ``750``:

   .. code-block:: console

      $ sudo chown root:cyb3rhq /var/ossec/active-response/bin/yara.sh
      $ sudo chmod 750 /var/ossec/active-response/bin/yara.sh

#. Add the following within the ``<syscheck>`` block of the Cyb3rhq agent ``/var/ossec/etc/ossec.conf`` configuration file to monitor the ``/home`` directory:
...
#. Create a script ``yara.py`` in the ``C:\Program Files (x86)\ossec-agent\active-response\bin\`` directory. This script runs a YARA scan against any file modified or added to the monitored directory. It also queries ChatGPT to enrich the logs and attempts to remove malware files detected by YARA. Replace ``<API_KEY>`` with your `OpenAI API key <https://platform.openai.com/docs/quickstart>`__ and ``<OPENAI_MODEL>`` with your preferred `OpenAI model <https://platform.openai.com/docs/models>`__. The model used in this POC guide is *gpt-4-turbo*:
   ...
   .. code-block:: python
      ...
      def query_chatgpt(description):
          ...
          if response.status_code == 200:
              ...
              log_message("cyb3rhq-YARA: ERROR - Invalid ChatGPT API key")
              return None
          else:
      ...
      def main():
          ...
          if yara_output is not None:
              ...
              if description:
                  ...
                  if chatgpt_response:
                      combined_output = f"cyb3rhq-YARA: INFO - Scan result: {yara_output} | chatgpt_response: {chatgpt_response}"
                      log_message(combined_output)
                  else:
                      # Log the Yara scan result without the ChatGPT response
                      log_message(f"cyb3rhq-YARA: INFO - Scan result: {yara_output} | chatgpt_response: None")

                  # Delete the scanned file if a description is found
                  try:
                      os.remove(syscheck_file_path)
                      if not os.path.exists(syscheck_file_path):
                          log_message(f"cyb3rhq-YARA: INFO - Successfully deleted {syscheck_file_path}")
                      else:
                          log_message(f"cyb3rhq-YARA: INFO - Unable to delete {syscheck_file_path}")
                  except Exception as e:
   ...
   .. note::
      ...
      If the supplied ``<API_KEY>`` is invalid, Cyb3rhq triggers an alert with the value of the ``chatgpt_response`` field set to ``None``. You can find logs about the invalid API key in the ``C:\Program Files (x86)\ossec-agent\active-response\active-response.log`` file.
...
#. Add the following within the ``<syscheck>`` block of the Cyb3rhq agent ``C:\Program Files (x86)\ossec-agent\ossec.conf`` configuration file to monitor the Users directory:
...
Perform the following steps on the Cyb3rhq server to configure custom rules, decoders, and the active response module.

#. Add the following decoders to the Cyb3rhq server ``/var/ossec/etc/decoders/local_decoder.xml`` file to parse the data in YARA scan results:
   ...
   .. code-block:: xml
      ...
      <decoder name="YARA_decoder">
        <prematch>cyb3rhq-YARA:</prematch>
      </decoder>

      <decoder name="YARA_child">
        <parent>YARA_decoder</parent>
        <regex type="pcre2">cyb3rhq-YARA: (\S+)</regex>
   ...
   .. code-block:: xml
      ...
      <group name="yara,">
        ...
        <rule id="108001" level="10">
          ...
          <match>cyb3rhq-YARA: INFO - Scan result: </match>
...
#. Add the following configuration to the Cyb3rhq server ``/var/ossec/etc/ossec.conf`` configuration file. This configures the active response module to trigger after the rules with ID ``100300``, ``100301``, ``100302``, and ``100303`` are fired:
...
.. code-block:: console
   ...
   # curl "https://cyb3rhq-demo.s3-us-west-1.amazonaws.com/mirai" > /home/mirai
   # curl "https://cyb3rhq-demo.s3-us-west-1.amazonaws.com/xbash" > /home/xbash
   # curl "https://cyb3rhq-demo.s3-us-west-1.amazonaws.com/webshell" > /home/webshell

You can visualize the alert data in the Cyb3rhq dashboard. To do this, go to the **Modules** > **Security events** tab and add the ``rule.groups:yara`` filter in the search bar to query the alerts.
...
.. code-block:: powershell
   ...
   > curl "https://cyb3rhq-demo.s3-us-west-1.amazonaws.com/mirai" -o   $env:USERPROFILE\Downloads\mirai
   > curl "https://cyb3rhq-demo.s3-us-west-1.amazonaws.com/xbash" -o   $env:USERPROFILE\Downloads\xbash
   > curl "https://cyb3rhq-demo.s3-us-west-1.amazonaws.com/webshell" -o $env:USERPROFILE\Downloads\webshell

You can visualize the alert data in the Cyb3rhq dashboard. To do this, go to the **Security events** module and add the filter in the search bar to query the alerts.
