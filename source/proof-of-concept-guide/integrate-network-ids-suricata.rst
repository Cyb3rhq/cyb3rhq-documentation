...
.. meta::
   :description: Cyb3rhq integrates with Suricata, a NIDS that detects threats by monitoring network traffic. Learn more about this in this PoC.

Network IDS integration
=======================

Cyb3rhq integrates with a network-based intrusion detection system (NIDS) to enhance threat detection by monitoring and analyzing network traffic.

In this use case, we demonstrate how to integrate Suricata with Cyb3rhq. Suricata can provide additional insights into your network's security with its network traffic inspection capabilities.
...
+===============+=================================================================================================================================================+
| Ubuntu 22.04  | This is the endpoint where you install Suricata. In this use case, Cyb3rhq monitors and analyzes the network traffic generated on this endpoint.  |
+---------------+-------------------------------------------------------------------------------------------------------------------------------------------------+

Configuration
-------------

Take the following steps to configure Suricata on the Ubuntu endpoint and send the generated logs to the Cyb3rhq server.
...
#. Add the following configuration to the ``/var/ossec/etc/ossec.conf`` file of the Cyb3rhq agent. This allows the Cyb3rhq agent to read the Suricata logs file:
...
Cyb3rhq automatically parses data from ``/var/log/suricata/eve.json`` and generates related alerts on the Cyb3rhq dashboard.

Ping the Ubuntu endpoint IP address from the Cyb3rhq server:
