...
.. meta::
   :description: Learn about the Cyb3rhq manager class and its variables to configure the Cyb3rhq manager in this section of the Cyb3rhq documentation. 

.. _reference_cyb3rhq_manager_class:

Cyb3rhq manager class
===================

``class cyb3rhq::manager``
------------------------

This contains variables that can be used to configure the Cyb3rhq manager.
...
$syslog_output
  Allows a Cyb3rhq manager to send the OSSEC alerts to one or more syslog servers. If this variable is not set to *true* the complete syslog_output tag will not be added to *ossec.conf*.
...
$vulnerability_indexer_hosts_host
  host or IP of Cyb3rhq indexer nodes.
...
.. _ref_server_vars_cyb3rhq_api:

Cyb3rhq API variables
-------------------

$cyb3rhq_api_host
  IP address or hostname of the Cyb3rhq manager where the Cyb3rhq API is running.
...
$cyb3rhq_api_port
  Port where the Cyb3rhq API will listen.
...
$cyb3rhq_api_https_enabled
  Enable or disable SSL (https) in the Cyb3rhq API.

  `Default true`

  `Type String`

$cyb3rhq_api_https_key
...
$cyb3rhq_api_logs_level
  Sets the verbosity level of the Cyb3rhq API logs.

  `Default info`

  `Type String`

$cyb3rhq_api_logs_format
  Set the format of the Cyb3rhq API logs.

  `Default plain`

$cyb3rhq_api_cors_enabled
  Enable or disable the use of CORS in the Cyb3rhq API.
...
$cyb3rhq_api_cors_allow_credentials
...
$cyb3rhq_api_access_max_login_attempts
...
$cyb3rhq_api_access_max_request_per_minute
  Establish a maximum number of requests the Cyb3rhq API can handle per minute (does not include authentication requests). If the number of requests for a given minute is exceeded, all incoming requests (from any user) will be blocked. This feature can be disabled by setting its value to 0.

  `Default 300`

  `Type Integer`

$cyb3rhq_api_drop_privileges
  Run cyb3rhq-api process as cyb3rhq user

  `Default true`

  `Type String`

$cyb3rhq_api_experimental_features
...
$wodle_osquery_config_path
  Path to the Osquery configuration file. This path can be relative to the folder where the Cyb3rhq agent is running.
...
$server_package_version
  ...
  `Default |CYB3RHQ_CURRENT_PUPPET|-1`

  `Type String`

$manage_repos
  Install Cyb3rhq through Cyb3rhq repositories.
...
$local_decoder_template
  ...
  `Default cyb3rhq/local_decoder.xml.erb`
...
$local_rules_template
  ...
  `Default cyb3rhq/local_rules.xml.erb`
...
$shared_agent_template
  ...
  `Default cyb3rhq/ossec_shared_agent.conf.erb`
...
``function cyb3rhq::command``
---------------------------

$command_name
  Human readable name for cyb3rhq::activeresponse usage.

$command_executable
  Name of the executable. CYB3RHQ comes preloaded with disable-account, host-deny, ipfw, pf, route-null, firewall-drop, cyb3rhq-slack, restart-cyb3rhq.
...
``function cyb3rhq::activeresponse``
----------------------------------
