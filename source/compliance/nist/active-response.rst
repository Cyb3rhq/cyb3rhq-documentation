...
The Cyb3rhq active response module performs autonomous actions on endpoints to mitigate security threats. You can  configure the module to automatically execute scripts when specific alerts trigger. These scripts execute actions, such as a firewall block or drop, traffic shaping or throttling, and account lockout.

The Cyb3rhq :doc:`active response </user-manual/capabilities/active-response/index>` module assists in meeting the following NIST 800-53 controls: 
...
The Cyb3rhq active response module assists in meeting the above controls by responding to brute force and denial of service attacks. Cyb3rhq includes out-of-the-box active response commands that drop traffic from a malicious IP address or disable a user account that is a victim of brute forcing.

Use case: Automatically disable an account
------------------------------------------

This use case shows how Cyb3rhq helps meet the NIST **AC-7 Unsuccessful logon attempts** requirement by identifying and taking precautionary responses to failed login attempts.

In this scenario, the Cyb3rhq active response module automatically disables a user account on a monitored Ubuntu 22.04 endpoint when events analysis detects multiple failed terminal login attempts. You can then track the alerts for these events and actions on the Cyb3rhq dashboard.

Cyb3rhq server
^^^^^^^^^^^^

#. Add the following configuration to the ``<ossec-config>`` block of the Cyb3rhq server configuration file ``/var/ossec/etc/ossec.conf``:
...
#. Attempt to log in with the wrong credentials to the ``<user2>`` account using ``<user1>`` account:
   ...
   The image below shows the related alerts on the Cyb3rhq dashboard:  

      .. thumbnail:: /images/compliance/nist/alerts-on-the-cyb3rhq-dashboard.png    
         :title: Alerts on the Cyb3rhq dashboard
         :alt: Alerts on the Cyb3rhq dashboard
         :align: center
         :width: 80%
