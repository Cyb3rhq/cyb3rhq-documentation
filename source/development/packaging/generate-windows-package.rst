...
.. meta::
  :description: Cyb3rhq provides an automated way of building Windows packages. Learn how to build your own Cyb3rhq Windows packages in this section of our documentation.
...
Cyb3rhq provides an automated way of building Windows packages.
...
Download our cyb3rhq repository from GitHub and go to the ``windows`` directory.

.. code-block:: console

    $ git clone https://github.com/cyb3rhq/cyb3rhq && cd cyb3rhq/packages/windows && git checkout v|CYB3RHQ_CURRENT_WINDOWS|
...
.. code-block:: none
  ...
  Usage: ./generate_compiled_windows_agent.sh [OPTIONS]
      ...
      -b, --branch <branch>     [Optional] Select Git branch to compile Cyb3rhq code.
      --sources <path>          [Optional] Absolute path containing cyb3rhq source code. This option will use local source code instead of downloading it from GitHub. By default: '../../src'.
...
Once you have obtained the zip with the compiled agent, You need to copy it along with ``generate_cyb3rhq_msi.ps1`` script to the Windows host.
...
Then copy the ``generate_cyb3rhq_msi.ps1`` script into the ``src/win32`` directory.

.. code-block:: console

  # cp generate_cyb3rhq_msi.ps1 .\[AGENT_UNCOMPRESSED_FOLDER]\src\win32

Execute the ``generate_cyb3rhq_msi.ps1`` script, with the different options you desire:

.. code-block:: console

  # cd .\[AGENT_UNCOMPRESSED_FOLDER]\src\win32
  # .\generate_cyb3rhq_msi.ps1
...
.. code-block:: none
  ...
  This tool can be used to generate the Windows Cyb3rhq agent msi package.
      ...
      USAGE:
          ./generate_cyb3rhq_msi.ps1  -MSI_NAME {{ NAME }} -SIGN {{ yes|no }} -WIX_TOOLS_PATH {{ PATH }} -SIGN_TOOLS_PATH {{ PATH }} -CERTIFICATE_PATH {{ PFX_CERT_PATH }} -CERTIFICATE_PASSWORD {{ PFX_CERT_PASSWORD }}

Below is an example of how to build a Windows MSI package.

.. code-block:: console

   # ./generate_cyb3rhq_msi.ps1 -MSI_NAME mypackage.msi -SIGN no

Here is an example of using a specific certificate and password.

.. code-block:: console

   # ./generate_cyb3rhq_msi.ps1 -MSI_NAME mypackage.msi -SIGN yes -CERTIFICATE_PATH .\certificate.pfx -CERTIFICATE_PASSWORD mypassword
...
.. code-block:: console
   ...
   # ./generate_cyb3rhq_msi.ps1 -MSI_NAME mypackage.msi -SIGN yes -WIX_TOOLS_PATH C:\path_to_wix_tools_binary_files -SIGN_TOOLS_PATH C:\path_to_sign_tools_binary_files
