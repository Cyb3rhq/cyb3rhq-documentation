...
.. meta::
    :description: Learn more about how to deploy a Cyb3rhq cluster: introduction, architecture overview, code structure and troubleshooting.

.. _dev-cyb3rhq-cluster:
...
Recommended reading: :doc:`Deploying a Cyb3rhq cluster </user-manual/manager/cyb3rhq-server-cluster>`.
...
Cyb3rhq's main workload is processing events from the agents and raise alerts. This is why all required information to receive events from the agents needs to be synchronized. This information is:
...
Having all this information synchronized, any cluster node is capable of processing and raising alerts from the agents, making it possible to horizontally scale a Cyb3rhq environment when new agents are added.

Architecture overview
---------------------

The following diagram shows a typical Cyb3rhq cluster architecture:

.. thumbnail:: ../images/manual/cluster/cluster-infrastructure.png
    :title: Cyb3rhq cluster architecture
    :alt: Cyb3rhq cluster architecture
    :align: center
    :width: 80%
...
.. note::
    The cyb3rhq cluster doesn't manage the load balancer.

Types of nodes
^^^^^^^^^^^^^^

There are two different types of nodes inside the Cyb3rhq cluster. These node types define the node's tasks inside the cluster and also, they define a hierarchy of nodes used to know which information prevails when doing synchronizations.
...
.. thumbnail:: ../images/manual/cluster/cluster-flow.png
    :title: Cyb3rhq cluster workflow
    :alt: Cyb3rhq cluster workflow
    :align: center
    :width: 80%
...
3. The master compares the received checksums with its own and creates three different groups of files:
   ...
   Then the master prepares a zip package with a JSON containing all this information and the required files the worker needs to update. The maximum zip size is specified in the ``max_zip_size`` variable of the `cluster.json <https://github.com/cyb3rhq/cyb3rhq/blob/v|CYB3RHQ_CURRENT|/framework/cyb3rhq/core/cluster/cluster.json>`__ file. In case it is exceeded, the remaining files will be synced in the next iteration of Integrity.
...
2. The worker asks to its local :ref:`cyb3rhq-db <cyb3rhq-db>` service for the information of agents marked as not synchronized.
3. The worker sends the master a JSON string containing the information retrieved from cyb3rhq-db.
4. The master sends the received information to its local cyb3rhq-db service, where it is updated.
...
2. The workers send the received information to their local :ref:`cyb3rhq-db <cyb3rhq-db>` service, where it is updated.
...
This thread is only executed by the master. It periodically asks to its local :ref:`cyb3rhq-db <cyb3rhq-db>` service new information (since the last time this task was run) of agent-groups. The task is not repeated until such information is sent to all worker nodes.
...
The cyb3rhq cluster protocol is defined on top of this framework. The following diagram shows all Python classes defined based on ``asyncio.Protocol``:

.. thumbnail:: ../images/development/cluster-protocol-handler.png
    :title: Cyb3rhq cluster protocol class inheritance
    :align: center
    :width: 80%

The higher classes on the diagram (``cyb3rhq.core.cluster.common.Handler``, ``cyb3rhq.core.cluster.server.AbstractServerHandler`` and ``cyb3rhq.core.cluster.client.AbstractClient``) define abstract concepts of what a client and a server is. Those abstract concepts are used by the lower classes on the diagram (``cyb3rhq.core.cluster.local_server.LocalServerHandler``, ``cyb3rhq.core.cluster.master.MasterHandler``, ``cyb3rhq.core.cluster.worker.WorkerHandler`` and ``cyb3rhq.core.cluster.local_client.LocalClientHandler``) to define specific communication protocols. These specific protocols are described in the `Protocols`_ section.
...
When the ``cyb3rhq-clusterd`` process starts in the master, it creates a ``Master`` object. Every time a new worker connects to the master, a ``MasterHandler`` object is created to handle the connection with that worker (incoming requests, synchronization processes, etc). That means there will always be at least a ``Master`` object and as many ``MasterHandler`` objects as connected workers. The ``Master`` object will be the one managing all ``MasterHandler`` objects created.

In the worker side, when the ``cyb3rhq-clusterd`` process starts it creates a ``Worker`` object. This object is in charge of initializing worker variables to connect to the master. A ``WorkerHandler`` object is created when connecting to the master node. This object will be the one sending requests to the master and managing synchronization processes.
...
The communication protocol used in all communications (both cluster and API) is defined in the ``cyb3rhq.core.cluster.common.Handler``. Each message in the protocol has the following structure:
...
Cyb3rhq cluster protocol
~~~~~~~~~~~~~~~~~~~~~~

This communication protocol is used by all cluster nodes to synchronize the necessary information to receive reports from the agents. All communications are made through TCP. These commands are defined in ``cyb3rhq.core.cluster.master.MasterHandler.process_request`` and in ``cyb3rhq.core.cluster.worker.WorkerHandler.process_request``.
...
This communication protocol is used by the API to forward requests to other cluster nodes. All communications are made using a Unix socket since the communication is all local (from the process running the API to the process running the cluster). These commands are defined in ``cyb3rhq.core.cluster.local_server.LocalServerHandler.process_request``, ``cyb3rhq.core.cluster.local_server.LocalServerHandlerMaster.process_request`` and ``cyb3rhq.core.cluster.local_server.LocalServerHandlerWorker.process_request``.
...
As said before, all protocols are built from a common abstract base. This base defines some messages to manage connections, keep alives, etc. These commands are defined in ``cyb3rhq.core.cluster.common.Handler.process_request``, ``cyb3rhq.core.cluster.server.AbstractServerHandler.process_request`` and ``cyb3rhq.core.cluster.client.AbstractClient.process_request``.
...
|                 |             |                    | The Cyb3rhq protocol modifies this command to add extra arguments.         |
+-----------------+-------------+--------------------+--------------------------------------------------------------------------+
...
Each of the "threads" described in the `Workflow`_ section are implemented as asynchronous tasks. These tasks are started in ``cyb3rhq.core.cluster.client.AbstractClientManager.start``, ``cyb3rhq.core.cluster.server.AbstractServer.start``, and ``cyb3rhq.core.cluster.local_server.LocalServer.start`` and they are all implemented using infinite loops.
...
Child processes are created when the parent `cyb3rhq-clusterd` starts. They stay in the process pool waiting for new jobs to be assigned to them. There are two child processes by default within the master node pool. This value can be changed in the `process_pool_size` variable in the `cluster.json <https://github.com/cyb3rhq/cyb3rhq/blob/v|CYB3RHQ_CURRENT|/framework/cyb3rhq/core/cluster/cluster.json>`__ file. The worker nodes, on the other hand, create a single child process and this number is not modifiable. The tasks that use multiprocessing in the cluster are the following.
...
* **1**: The worker's ``sync_integrity`` task wakes up after sleeping during *interval* seconds (which is defined in the `cluster.json <https://github.com/cyb3rhq/cyb3rhq/blob/v|CYB3RHQ_CURRENT|/framework/cyb3rhq/core/cluster/cluster.json>`__ file). The first thing it does is checking whether the previous synchronization process is finished or not using the ``syn_i_w_m_p`` command. The master replies with a boolean value specifying that the previous synchronization process is finished and, therefore, the worker can start a new one.
...
The type association with every endpoint can be found here: `API controllers <https://github.com/cyb3rhq/cyb3rhq/tree/v|CYB3RHQ_CURRENT|/api/api/controllers>`__.
...
The cluster has lots of different components working together: a network protocol, I/O and some Cyb3rhq specific logic. All these components log their progress in ``logs/cluster.log`` file. To make things easier for the developer, each component includes a log tag to help the developer see which exact component logged the event. The following is an example of how the log file looks:
...
.. code-block:: none
    ...
    2019/04/10 15:37:58 cyb3rhq-clusterd: ERROR: [Cluster] [Main] Could not get checksum of file client.keys: [Errno 13] Permission denied: '/var/ossec/etc/client.keys'
...
.. code-block:: console
    ...
    # sed -i "s:cyb3rhq_clusterd.debug=1:cyb3rhq_clusterd.debug=2:g" /var/ossec/etc/internal_options.conf
...
.. code-block:: none
    ...
    2019/04/10 15:50:37 cyb3rhq-clusterd: ERROR: [Cluster] [Main] Could not get checksum of file client.keys: [Errno 13] Permission denied: '/var/ossec/etc/client.keys'
    Traceback (most recent call last):
    File "/var/ossec/framework/python/lib/python3.9/site-packages/cyb3rhq-|CYB3RHQ_CURRENT|-py3.7.egg/cyb3rhq/core/cluster/cluster.py", line 217, in walk_dir
        entry_metadata['blake2b'] = blake2b(os.path.join(common.ossec_path, full_path))
    File "/var/ossec/framework/python/lib/python3.9/site-packages/cyb3rhq-|CYB3RHQ_CURRENT|-py3.7.egg/cyb3rhq/core/utils.py", line 555, in blake2b
...
* Modifying the ``cyb3rhq_clusterd.debug`` variable in the ``internal_options.conf`` file.
* Using the argument ``-d`` in the ``cyb3rhq-clusterd`` binary.
