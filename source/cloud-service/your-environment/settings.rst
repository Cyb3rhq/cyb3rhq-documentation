...
.. meta::
  :description: Learn about Cyb3rhq Cloud settings. 
...
This basic setting sets the maximum count of active Cyb3rhq agents that the environment can support. Please note that while registering an unlimited number of Cyb3rhq agents is possible, the active agent count is limited by this setting.
...
The indexed data was previously known as hot storage. It includes the data available on the Cyb3rhq dashboard, which corresponds to the information indexed by Cyb3rhq. This information becomes searchable and analyzable as soon as Cyb3rhq ingests and indexes the events sent by the agents.
...
The environment is configured with the `limits eps option <https://cyb3rhq.github.io/documentation/current/user-manual/reference/ossec-conf/global.html#limits>`_ using the following parameters:
...
You can update your environment settings directly in the Cyb3rhq Cloud Console under the environment details page. Here's how the process works:
